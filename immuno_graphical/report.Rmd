...
neut.marker.captions <- switch(study_name, 
                               ...
                               "VAT08"="The dashed lines in each figure is LLOD for neutralizing antibody assays.",
...
for (bsero in c("BaselineNeg", "BaselinePos")) {
  
  bseroLabel <- ifelse(bsero=="BaselineNeg", "negative", "positive")
  
  for (trt in c("vaccine","placebo", if(study_name=="VAT08") "pooled")) {  
    
    if (study_name!="VAT08" && trt=="placebo" && bsero=="BaselineNeg") next # only report for placebo baseline neg for VAT08
...
for (trt in c("trt_both", "trt_vaccine", if(study_name=="VAT08") "trt_placebo")){
  ...
  for (bsero in c("bstatus_both", "bstatus_Neg", "bstatus_Pos")){
    ...
    for(tp in times[!times %in% c("B",paste0("Delta",timepoints[length(timepoints)],"over",timepoints[1]))]) {
      ...
      if (study_name=="VAT08" && grepl("Detla", tp)) next # only report main timepoints for VAT08
...
### (VAT08) Baseline neg/pos vs. vaccine/placebo
### Vaccine vs. Placebo

for (bsero in c("BaselineNeg", "BaselinePos", if(study_name=="VAT08") "bstatus")) { #only VAT08 report Baseline neg/pos vs. vaccine/placebo
