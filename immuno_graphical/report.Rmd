        ...
        for (country in c("Nvx_US_Mex", if(attr(config,"config")=="prevent19") "Nvx_US")){
            ...
            for (asy in assay_immuno){
                ...
                for (times_index in c(""#, if (study_name == "NextGen_Mock") "_2"
                    ...
                    specimen_type <- paste0("based on ", ifelse(grepl("sera", asy), "sera", ifelse(grepl("nasal", asy), "nasal lining", ifelse(grepl("saliva", asy), "saliva", "PBMC"))))
 ...
 for(tp in if (study_name == "NextGen_Mock") {c(paste0(tps_no_fold_change, "_initial"), paste0(tps_no_fold_change[c(1,2,4)], "_final"))} else {times_[!times_ %in% c("B",paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]}) {
    ...
    for (trt in c("trt_both", paste0("trt_", trt.labels.2[2]), if (study_name %in% c("VAT08")) paste0("trt_", trt.labels.2[1]))){
        ...
        for (bsero in c("bstatus_both", "bstatus_Neg", "bstatus_Pos")){
            ...
            } else if (study_name == "NextGen_Mock") {c("bAb_IgG_sera", "bAb_IgA_sera", "nAb_sera", 
                                   # "bAb_IgG_nasal", "bAb_IgA_nasal", "nAb_nasal", "bAb_IgG_saliva", "bAb_IgA_saliva" "nAb_saliva",
                                                        "ics")
...
for (bsero in c(bstatus.labels.2, if(study_name=="VAT08") c("bstatus"#, "bstatus_gender"
    ...
    for(tp in times_[!times_ %in% c(paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
        ...
        specimen_type <- paste0("based on ", ifelse(grepl("sera", pn), "sera", ifelse(grepl("nasal", pn), "nasal lining", ifelse(grepl("saliva", pn), "saliva", "PBMC"))))
...
if(study_name %in% c("VAT08", "NextGen_Mock") | attr(config,"config")=="janssen_partA_VL"){
    ...
    c("bind.*sera", #"bind.*nasal", #"bind.*saliva", 
...
for (tp in if (study_name == "NextGen_Mock") {tps_no_fold_change} else {tps_no_delta_over_tinterm}){
    for (a in if (study_name == "NextGen_Mock") {c("IgG_sera", "pseudoneutid50_sera", "T4", "T8")
        ...
        #    TRUE ~ immuno_postfix_label) 
        specimen_type <- paste0("based on ", ifelse(grepl("sera", a), "sera", ifelse(grepl("nasal", a), "nasal lining", ifelse(grepl("saliva", a), "saliva", "PBMC"))))
...
for (assay_grp in if (study_name == "NextGen_Mock") {
    ...
    specimen_type <- paste0("based on ", ifelse(grepl("sera", assay_grp), "sera", ifelse(grepl("nasal", assay_grp), "nasal lining", ifelse(grepl("saliva", assay_grp), "saliva", "PBMC"))))
    ...
    for (trt in c("vac_pla", if (study_name == "NextGen_Mock") "vac", if (study_name == "NextGen_Mock") "pla")){
        ...
        for (tp in tps_no_fold_change){
            ...
            for (asy in if (study_name != "NextGen_Mock") {""} else {
                ...
                #    study_name == "NextGen_Mock" & tp %in% c("Day91", "Day366") ~ " (Track A RIS-PBMC cohort)",
                #    TRUE ~ immuno_postfix_label)
                specimen_type <- paste0("based on ", ifelse(grepl("sera", asy) & !grepl("T4|T8", asy), "sera", ifelse(grepl("nasal", asy) & !grepl("T4|T8", asy), "nasal lining", ifelse(grepl("saliva", asy) & !grepl("T4|T8", asy), "saliva", ifelse(grepl("T4|T8", asy) & !grepl("sera|nasal|saliva", asy), "PBMC", "")))))
