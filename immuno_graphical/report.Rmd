...
if (study_name == "NextGen_Mock") {
    ...
    nextgen_output_flag = 2 # sera_only_ics, as default with assay_metadata filtered to sera only assays, 
                            # options: 3 (whole_report)  1 (BARDA_TLF)
}
```
        ...
        for (bsero in c("bstatus_both", "bstatus_Neg", "bstatus_Pos")){
            ...
            for (asy in if(study_name=="VAT08" | attr(config,"config")=="janssen_partA_VL") {c("bAb","nAb")
              ...
              c(if (nextgen_output_flag >= 2) assays[grepl("bind.*sera", assays)],
                ...
                if (nextgen_output_flag >= 2) "pseudoneutid50_sera_KP.2",
                ...
                for (reg in if (attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}) {
                    ...
                    specimen_type <- paste0("based on ", ifelse(grepl("sera", asy), "sera", ifelse(grepl("nasal", asy), "nasal fluid", ifelse(grepl("saliva", asy), "saliva", "PBMC"))))
                    cohort_name <- ifelse(grepl("sera", asy), "(RIS-sera). ", ifelse(grepl("nasal", asy), "(RIS-nasal). ", ifelse(grepl("saliva", asy), "(RIS-saliva). ", "(RIS-PBMC). ")))
                    cohort_name <- ifelse(grepl("initial", tp), "Track A RIS-PBMC", cohort_name)#gsub("\\(", "(Track A ", cohort_name), cohort_name)
...
for (bsero in c(bstatus.labels.2, if(study_name=="VAT08") c("bstatus"#, "bstatus_gender"
    ...
    for(tp in times_[!times_ %in% c(paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
        ...
        cohort_name <- ifelse(tp %in% c("Day91", "Day366"), "Track A RIS-PBMC", cohort_name)#gsub("\\(", "(Track A ", cohort_name), cohort_name)
...
if(study_name %in% c("VAT08", "NextGen_Mock") | attr(config,"config")=="janssen_partA_VL"){
    ...
    c(if (nextgen_output_flag >= 1) "bind.*sera", 
      if (nextgen_output_flag == 3) c("bind.*nasal", "bind.*saliva"), 
      if (nextgen_output_flag >= 1) "pseudo.*sera", 
      if (nextgen_output_flag == 3) c("pseudo.*nasal", "pseudo.*saliva"), 
      if (nextgen_output_flag >= 1) "ics")
    } else {c("bind", "pseudo")}){
        ...
        for (tp in #if(study_name=="VAT08") {c("Day", "Delta")} else {
             c(if(attr(config,"config") != "nextgen_mock") "Day", 
               if(attr(config,"config") == "nextgen_mock" & nextgen_output_flag >= 2) "Day initial", # "Day91", "Day366" Track A
...
for (assay_grp in if (study_name == "NextGen_Mock") {
    ...
    cohort_name <- "Track A RIS-PBMC" #gsub("\\(", "(Track A ", cohort_name)
...
for (cohort in if (study_name != "NextGen_Mock") {""} else {c("_initial", "_final")}){
    ...
    cohort_name <- ifelse(grepl("initial", cohort), "Track A RIS-PBMC", cohort_name)#gsub("\\(", "(Track A ", cohort_name), cohort_name)
