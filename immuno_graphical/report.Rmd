    ...
    for (trt in c(trt.labels.2[2:1], if(study_name=="VAT08") "pooled")) {  
        ...
        for (country in c("Nvx_US_Mex", if(attr(config,"config")=="prevent19") "Nvx_US")){
            ...
            if (study_name == "VaxArt_Mock" & nextgen_output_flag == 1) {
              assay_immuno_ = "pseudoneutid50_sera_XBB.1.5"
            ...
            for (asy in assay_immuno_){
                ...
                for (times_index in c(""#, if (study_name == "VaxArt_Mock") "_2"
                    ...
                    #    study_name == "VaxArt_Mock" & grepl("T4|T8", asy) & !grepl("bind|pseudo", asy) ~ " (RIS-PBMC cohort)",
                    #    study_name == "VaxArt_Mock" & !grepl("T4|T8", asy) & grepl("bind|pseudo", asy) ~ " (RIS cohort)",
                        #study_name == "VaxArt_Mock" & !grepl("T4|T8", asy) & grepl("bind", asy) ~ " (RIS or RIS-PBMC cohort)",
                    ...
                    cohort_name <- ifelse(grepl("sera", asy), "(RIS-sera). ", ifelse(grepl("nasal", asy), "(RIS-nasal). ", ifelse(grepl("saliva", asy), "(RIS-saliva). ", "(RIS-PBMC). ")))
        ...
        for (bsero in c("bstatus_both", "bstatus_Neg", "bstatus_Pos")){
            ...
            for (asy in if(study_name=="VAT08" | attr(config,"config")=="janssen_partA_VL") {c("bAb","nAb")
              ...
              c(if (nextgen_output_flag >= 2) assays[grepl("bind.*sera", assays)],
                ...
                if (nextgen_output_flag == 1) "pseudoneutid50_sera_XBB.1.5",
                ...
                for (reg in if (attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}) {
                    ...
                    #immuno_postfix_label <- case_when(
                    #    study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366") ~ " (Track A RIS-PBMC cohort)",
                    #    study_name == "VaxArt_Mock" & asy %in% c("bAb", "nAb") ~ " (RIS cohort)",
                    #    study_name == "VaxArt_Mock" & asy == "ics" ~ " (RIS-PBMC cohort)",
                    #    TRUE ~ immuno_postfix_label)
                    specimen_type <- paste0("based on ", ifelse(grepl("sera", asy), "sera", ifelse(grepl("nasal", asy), "nasal fluid", ifelse(grepl("saliva", asy), "saliva", "PBMC"))))
                    cohort_name <- ifelse(grepl("sera", asy), "(RIS-sera). ", ifelse(grepl("nasal", asy), "(RIS-nasal). ", ifelse(grepl("saliva", asy), "(RIS-saliva). ", "(RIS-PBMC). ")))
...
for (bsero in c(bstatus.labels.2, if(study_name=="VAT08") c("bstatus"#, "bstatus_gender"
    ...
    for(tp in times_[!times_ %in% c(paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
        ...
        #immuno_postfix_label <- ifelse(study_name == "VaxArt_Mock" & tp %in% c("B", "Day31", "Day181"), " (RIS or RIS-PBMC cohort)",
        #                               ifelse(study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366"), " (Track A RIS-PBMC cohort)",
        ...
        cohort_name <- ifelse(grepl("sera", pn), "(RIS-sera). ", ifelse(grepl("nasal", pn), "(RIS-nasal). ", ifelse(grepl("saliva", pn), "(RIS-saliva). ", "(RIS-PBMC). ")))
            ...
            for (bsero in bstatus.labels.2) {
                ...
                for (trt in if (study_name == "VaxArt_Mock") "trt_comparison" else {trt.labels.2[2:1]}) {
                    ...
                    for (reg in if(attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}){
                        ...
                        #immuno_postfix_label <- case_when(
                        #    study_name == "VaxArt_Mock" & tp %in% c("Day initial") ~ " (Track A RIS-PBMC cohort)",
                        #    study_name == "VaxArt_Mock" & asy %in% c("bAb", "nAb") ~ " (RIS cohort)",
                        #    study_name == "VaxArt_Mock" & asy == "ics" ~ " (RIS-PBMC cohort)",
                        #    TRUE ~ immuno_postfix_label)
                        specimen_type <- paste0("based on ", ifelse(grepl("sera", asy), "sera", ifelse(grepl("nasal", asy), "nasal fluid", ifelse(grepl("saliva", asy), "saliva", "PBMC"))))
...
for (tp in if (study_name == "VaxArt_Mock") {tps_no_fold_change} else {tps_no_delta_over_tinterm}){
    for (a in if (study_name == "VaxArt_Mock") {c(
      ...
      "T4", "T8")
        ...
        #    study_name == "VaxArt_Mock" & asy == "tcell" ~ " (RIS-PBMC cohort)",
        #    TRUE ~ immuno_postfix_label) 
        ...
        cohort_name <- ifelse(grepl("sera", a), "(RIS-sera). ", ifelse(grepl("nasal", a), "(RIS-nasal). ", ifelse(grepl("saliva", asy), "(RIS-saliva). ", "(RIS-PBMC). ")))
...
for (assay_grp in if (study_name == "VaxArt_Mock") {
    ...
    details = ""
    details_lb = ""
    #immuno_postfix_label <- ifelse(study_name == "VaxArt_Mock", " (RIS or RIS-PBMC cohort at Day01, Day31, Day181, Track A RIS-PBMC cohort at Day91, Day366)", immuno_postfix_label)
    ...
    cohort_name <- ifelse(grepl("sera", assay_grp), "(RIS-sera). ", ifelse(grepl("nasal", assay_grp), "(RIS-nasal). ", ifelse(grepl("saliva", assay_grp), "(RIS-saliva). ", "(RIS-PBMC). ")))
    ...
    for (trt in c("vac_pla", if (study_name == "VaxArt_Mock") "vac", if (study_name == "VaxArt_Mock") "pla")){
        ...
        for (tp in tps_no_fold_change){
            ...
            for (asy in if (study_name != "VaxArt_Mock") {""} else {
                ...
                #immuno_postfix_label <- case_when(
                #    study_name == "VaxArt_Mock" & tp %in% c("B", "Day31", "Day181") ~ " (RIS or RIS-PBMC cohort)",
                #    study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366") ~ " (Track A RIS-PBMC cohort)",
                #    TRUE ~ immuno_postfix_label)
                ...
                cohort_name <- ifelse(grepl("sera", asy) & !grepl("T4|T8", asy), "(RIS-sera). ", ifelse(grepl("nasal", asy) & !grepl("T4|T8", asy), "(RIS-nasal). ", ifelse(grepl("saliva", asy) & !grepl("T4|T8", asy), "(RIS-saliva). ", ifelse(grepl("T4|T8", asy) & !grepl("sera|nasal|saliva", asy), "(RIS-PBMC). ", paste0("(RIS-", str_extract(asy, "sera|saliva|nasal"), ", RIS-PBMC). ") ))))
...
for (cohort in if (study_name != "VaxArt_Mock") {""} else {c("_initial", "_final")}){
    ...
    if (study_name == "VaxArt_Mock" & nextgen_output_flag == 1 & a != "pseudoneutid50_sera_XBB.1.5") next
    #immuno_postfix_label <- case_when(
    #    study_name == "VaxArt_Mock" & grepl("bind|pseudo", a) ~ " (RIS cohort)",
    #    study_name == "VaxArt_Mock" & grepl("T4|T8", a) ~ " (RIS-PBMC cohort)",
    #    TRUE ~ immuno_postfix_label)
    specimen_type <- paste0("based on ", ifelse(grepl("sera", a), "sera", ifelse(grepl("nasal", a), "nasal fluid", ifelse(grepl("saliva", a), "saliva", "PBMC"))))
    cohort_name <- ifelse(grepl("sera", a), "(RIS-sera). ", ifelse(grepl("nasal", a), "(RIS-nasal). ", ifelse(grepl("saliva", a), "(RIS-saliva). ", "(RIS-PBMC). ")))
