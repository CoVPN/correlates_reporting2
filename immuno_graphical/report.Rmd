    ...
    for (country in c("Nvx_US_Mex", if(attr(config,"config")=="prevent19") "Nvx_US")){
      ...
      for (asy in assay_immuno){
        ...
        for (times_index in c("", if (study_name == "NextGen_Mock") "_2")){
          ...
          immuno_postfix_label <- ifelse(study_name == "NextGen_Mock" & times_index == "", " (Whole RIS/RIS-PBMC)",
                                         ifelse(study_name == "NextGen_Mock" & times_index == "_2", " (Track A RIS/RIS-PBMC)",
...
for (trt in c("trt_both", "trt_vaccine", if (study_name %in% c("VAT08", "NextGen_Mock")) "trt_placebo")){
  ...
  for (bsero in c("bstatus_both", "bstatus_Neg", "bstatus_Pos")){
    ...
    for(tp in times_[!times_ %in% c(if (study_name != "NextGen_Mock") "B",paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
      ...
      for (asy in if(study_name=="VAT08" | attr(config,"config")=="janssen_partA_VL") {c("bAb","nAb")
        } else if (study_name == "NextGen_Mock") {c("bAb", "nAb", "ics")
        ...
        for (reg in if (attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}) {
          ...
          immuno_postfix_label <- ifelse(study_name == "NextGen_Mock" & tp %in% c("B", "Day31", "Day181"), " (Whole RIS/RIS-PBMC)",
                                         ifelse(study_name == "NextGen_Mock" & tp %in% c("Day91", "Day366"), " (Track A RIS/RIS-PBMC)",
                                                immuno_postfix_label))      
...
if(study_name %in% c("VAT08", "NextGen_Mock") | attr(config,"config")=="janssen_partA_VL"){
  for (tp in #if(study_name=="VAT08") {c("day", "delta")} else {
    ...
    for (asy in c("bAb" ,"nAb", if (study_name == "NextGen_Mock") "ics")){
      if (study_name == "NextGen_Mock" & tp == "day whole" & asy == "ics") next # do not have these plots
      
      for (bsero in c("Neg", "Pos")) {
        if (study_name == "NextGen_Mock" & bsero == "Neg") next # NextGen_Mock only has non-naive
        ...
        for (trt in c("placebo", "vaccine")) {
          ...
          for (reg in if(attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}){
            ...
            immuno_postfix_label <- ifelse(study_name == "NextGen_Mock" & tp %in% c("day whole"), " (Whole RIS/RIS-PBMC)",
                                         ifelse(study_name == "NextGen_Mock" & tp %in% c("day initial"), " (Track A RIS/RIS-PBMC)",
                                                immuno_postfix_label)) 
...
for (a in if (study_name == "NextGen_Mock") {c("bindN", "bindSpike_IgG_sera", "bindSpike_IgA_sera", "pseudoneutid50_sera", "T4", "T8")
...
for (assay_grp in if (study_name == "NextGen_Mock") {
  c(paste(assays[!grepl("mdw", assays)][c(TRUE, FALSE)], 
          assays[!grepl("mdw", assays)][c(FALSE, TRUE)], sep = "_"))
  } else {c("bAb","bAb_mdw","nAb","nAb_mdw")}){
    details = ""
    details_lb = ""
    immuno_postfix_label <- ifelse(study_name == "NextGen_Mock", " (Whole RIS/RIS-PBMC at B, Day31, Day181, Track A RIS/RIS-PBMC at Day91, Day366)",
                                                immuno_postfix_label)
...
for (a in assays){
  ...
  immuno_postfix_label <- ifelse(study_name == "NextGen_Mock", " (Whole RIS/RIS-PBMC)", immuno_postfix_label)
