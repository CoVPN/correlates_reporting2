...
if(include_bindN && any(grepl("bind", assays)) && !any(grepl("bindN", assays)) && !grepl("janssen_.+partA.*", attr(config,"config")) && !grepl("vat08|nextgen", attr(config,"config"))){
...
if (study_name == "VaxArt_Mock") {
    ...
    nextgen_output_flag = 1 # sera_only_ics, as default with assay_metadata filtered to sera only assays, 
                            # options: 3 (whole_report)  1 (BARDA_TLF) 2 (sera only + ics)
}
```
...
neut.marker.captions <- switch(study_name, 
                               ...
                               "VaxArt_Mock"="The dashed lines in each figure are ULOQ and LLOQ, from top to bottom respectively.")
...
\clearpage
## Pairs plots of antibody markers for specimen-specific cohorts
        ...
        for (country in c("Nvx_US_Mex", if(attr(config,"config")=="prevent19") "Nvx_US")){
            ...
            if (if.pair==1){
                ...
                for(tp in times_[!times_ %in% c(paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
                    ...
                    if (study_name == "VaxArt_Mock") next # no single timepoint plot for NextGen
            ...
            if (study_name == "VaxArt_Mock" & nextgen_output_flag == 1) {
              assay_immuno_ = "pseudoneutid50_sera_KP.2"
              
            } else {assay_immuno_ = assay_immuno}
          
            if (study_name == "VaxArt_Mock" & nextgen_output_flag == 1 & trt == "comparator_vaccine") next
          
            for (asy in assay_immuno_){
                
                for (times_index in c(""#, if (study_name == "VaxArt_Mock") "_2"
                                      )){
                    
                    if (study_name == "VaxArt_Mock" & bsero == bstatus.labels.2[1]) next # no naive for VaxArt_Mock
                    ...
                    #    study_name == "VaxArt_Mock" & grepl("T4|T8", asy) & !grepl("bind|pseudo", asy) ~ " (RIS-PBMCAS cohort)",
                    #    study_name == "VaxArt_Mock" & !grepl("T4|T8", asy) & grepl("bind|pseudo", asy) ~ " (RIS cohort)",
                        #study_name == "VaxArt_Mock" & !grepl("T4|T8", asy) & grepl("bind", asy) ~ " (RIS or RIS-PBMCAS cohort)",
                    #    TRUE ~ immuno_postfix_label)
                    specimen_type <- paste0("based on ", ifelse(grepl("sera", asy), "sera", ifelse(grepl("nasal", asy), "nasal fluid", ifelse(grepl("saliva", asy), "saliva", "PBMC"))))
                    cohort_name <- ifelse(grepl("sera", asy), "(RIS-seraAS). ", ifelse(grepl("nasal", asy), "(RIS-nasalAS). ", ifelse(grepl("saliva", asy), "(RIS-salivaAS). ", "(RIS-PBMCAS). ")))
                    
                    bseroLabel <- ifelse(study_name == "VaxArt_Mock", "", bseroLabel)
...
\clearpage
## RCDF plots of antibody markers for specimen-specific cohorts
```{r, echo=FALSE, results='asis'}
...
if (attr(config,"config") %in% c("vat08_combined","prevent19_stage2")){
 ...
 for(tp in if (study_name == "VaxArt_Mock") {#c(paste0(tps_no_fold_change, "_initial"), paste0(tps_no_fold_change[c(1,2,4)], "_final"))
   c(#"_initial", 
     "_final", "_final_v2")} else {times_[!times_ %in% c("B",paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]}) {

    for (trt in c("trt_both", paste0("trt_", trt.labels.2[2]), if (study_name %in% c("VAT08")) paste0("trt_", trt.labels.2[1]))){
        ...
        for (bsero in c("bstatus_both", "bstatus_Neg", "bstatus_Pos")){
            ...
            if ((bsero != "bstatus_Pos"| trt != "trt_both") & study_name == "VaxArt_Mock") next # VaxArt_Mock only have bstatus_Pos, trt_both plot
            if (trt=="trt_both" && bsero!="bstatus_both" && !study_name %in% c("VAT08", "VaxArt_Mock")) next # trt_both is only for bstatus both if the study is VAT08
            ...
            for (asy in if(study_name=="VAT08" | attr(config,"config")=="janssen_partA_VL") {c("bAb","nAb")
              ...
              c(if (nextgen_output_flag >= 2) assays[grepl("bind.*sera", assays)],
                if (nextgen_output_flag >= 2) "pseudoneutid50_sera",
                if (nextgen_output_flag == 1) "pseudoneutid50_sera_KP.2",
                #if (nextgen_output_flag >= 2) "pseudoneutid50_sera_mdw",
                if (nextgen_output_flag == 3) assays[grepl("nasal|saliva", assays)],
                if (nextgen_output_flag >= 2) assays[grepl("T4|T8", assays)])
                ...
                for (reg in if (attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}) {
                    ...
                    #immuno_postfix_label <- case_when(
                    #    study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366") ~ " (Track A RIS-PBMCAS cohort)",
                    #    study_name == "VaxArt_Mock" & asy %in% c("bAb", "nAb") ~ " (RIS cohort)",
                    #    study_name == "VaxArt_Mock" & asy == "ics" ~ " (RIS-PBMCAS cohort)",
                    #    TRUE ~ immuno_postfix_label)
                    specimen_type <- paste0("based on ", ifelse(grepl("sera", asy), "sera", ifelse(grepl("nasal", asy), "nasal fluid", ifelse(grepl("saliva", asy), "saliva", "PBMC"))))
                    cohort_name <- ifelse(grepl("sera", asy), "(RIS-seraAS). ", ifelse(grepl("nasal", asy), "(RIS-nasalAS). ", ifelse(grepl("saliva", asy), "(RIS-salivaAS). ", "(RIS-PBMCAS). ")))
                    cohort_name <- ifelse(grepl("initial", tp), gsub("\\(", "(Track A ", cohort_name), cohort_name)
...
if (!study_name  %in% c("VAT08", "VaxArt_Mock")) { # VAT08 and VaxArt_Mock don't report this
    for (bsero in bstatus.labels.2){
...
```{r, echo=FALSE, results='asis', eval = study_name!="VaxArt_Mock"}
## Box plots of antibody markers for specimen-specific cohorts
...
for (bsero in c(bstatus.labels.2, if(study_name=="VAT08") c("bstatus"#, "bstatus_gender"
    ...
    if (attr(config,"config")=="VaxArt_Mock" & bsero==bstatus.labels.2[1]) next # VaxArt_Mock only has non-naive
    ...
    for(tp in times_[!times_ %in% c(paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
        ...
        for (pn in if (study_name != "VaxArt_Mock") {""} else {c("IgG_sera", "IgA_nasal", "pseudoneutid50_sera",
                                                                  #"IgG_nasal", "pseudoneutid50_nasal", 
                                                              #"IgG_saliva", "IgA_saliva", "pseudoneutid50_saliva",
                                                                  "T4_T8")}) {
        
        if (study_name %in% c("VAT08", "VaxArt_Mock") && grepl("Delta", tp)) next # only report main timepoints for VAT08, VaxArt_Mock
        if (study_name == "VaxArt_Mock" & bsero == bstatus.labels.2[1]) next # VaxArt_Mock doesn't have naive ppts
        
        tpLabel <- gsub("ay ", "", labels.time[tp])
        
        #immuno_postfix_label <- ifelse(study_name == "VaxArt_Mock" & tp %in% c("B", "Day31", "Day181"), " (RIS or RIS-PBMCAS cohort)",
        #                               ifelse(study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366"), " (Track A RIS-PBMCAS cohort)",
        #                                      immuno_postfix_label)) 
        #bseroLabel <- ifelse(study_name == "VaxArt_Mock", "", bseroLabel)
        
        specimen_type <- paste0("based on ", ifelse(grepl("sera", pn), "sera", ifelse(grepl("nasal", pn), "nasal fluid", ifelse(grepl("saliva", pn), "saliva", "PBMC"))))
        cohort_name <- ifelse(grepl("sera", pn), "(RIS-seraAS). ", ifelse(grepl("nasal", pn), "(RIS-nasalAS). ", ifelse(grepl("saliva", pn), "(RIS-salivaAS). ", "(RIS-PBMCAS). ")))
        cohort_name <- ifelse(tp %in% c("Day91", "Day366"), gsub("\\(", "(Track A ", cohort_name), cohort_name)
...
```{r, fig.cap = paste0("Spaghetti plots of Ab markers over time: baseline negative vaccine + placebo arm"), eval = any(c("B", paste0("Day",timepoints[1]), "Day57") %in% times & study_name!="VAT08" & study_name!="ENSEMBLE" & study_name!="PREVENT19" & study_name!="VaxArt_Mock")}
...
```{r, fig.cap = paste0("Spaghetti plots of Ab markers over time: baseline positive vaccine + placebo arm"), eval = any(c("B", paste0("Day",timepoints[1]), "Day57") %in% times & if.pos.exist==1 & study_name!="VAT08" & study_name!="ENSEMBLE" & study_name!="PREVENT19" & study_name!="VaxArt_Mock")}
include_graphics(here(
    "immuno_graphical", "output", attr(config,"config"),
...
## Spider plots of antibody markers for specimen-specific cohorts
```{r, echo=FALSE, results='asis'}
## Spider plot of antibody markers for overall per-protocol cohort

if(study_name %in% c("VAT08", "VaxArt_Mock") | attr(config,"config")=="janssen_partA_VL"){
    
    for (asy in if(study_name == "VaxArt_Mock") {
    c(if (nextgen_output_flag >= 1) "bind_sera", 
      if (nextgen_output_flag == 3) c("bind_nasal", "bind_saliva"), 
      #if (nextgen_output_flag >= 1) "pseudo_sera", 
      #if (nextgen_output_flag == 3) c("pseudo_nasal", "pseudo_saliva"), 
      if (nextgen_output_flag >= 1) "T4",
      if (nextgen_output_flag >= 2) "T8")
    } else {c("bind", "pseudo")}){
        #if (study_name == "VaxArt_Mock" & tp %in% c("Day whole", "Day initial") & asy == "ics") next # do not have these plots
        
        for (tp in #if(study_name=="VAT08") {c("Day", "Delta")} else {
             c(if(attr(config,"config") != "nextgen_mock") "Day", 
               if(attr(config,"config") == "nextgen_mock") c(if (nextgen_output_flag >= 2) "Day initial", if (nextgen_output_flag >= 1) "Day whole")
               #, if (attr(config,"config")!="janssen_partA_VL") "Delta"
             )#}
        ) {
        
            for (bsero in bstatus.labels.2) {
                if (study_name == "VaxArt_Mock" & bsero == bstatus.labels.2[1]) next # VaxArt_Mock only has non-naive
                
                for (trt in if (study_name == "VaxArt_Mock") "trt_comparison" else {trt.labels.2[2:1]}) {
                    ...
                    for (reg in if(attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}){
                        ...
                        #immuno_postfix_label <- case_when(
                        #    study_name == "VaxArt_Mock" & tp %in% c("Day initial") ~ " (Track A RIS-PBMCAS cohort)",
                        #    study_name == "VaxArt_Mock" & asy %in% c("bAb", "nAb") ~ " (RIS cohort)",
                        #    study_name == "VaxArt_Mock" & asy == "ics" ~ " (RIS-PBMCAS cohort)",
                        #    TRUE ~ immuno_postfix_label)
                        specimen_type <- paste0("based on ", ifelse(grepl("sera", asy), "sera", ifelse(grepl("nasal", asy), "nasal fluid", ifelse(grepl("saliva", asy), "saliva", "PBMC"))))
                        unit_type <- ifelse(grepl("sera", asy), "serum", ifelse(grepl("nasal", asy), "nasal", ifelse(grepl("saliva", asy), "saliva", "PBMC")))
                        cohort_name <- ifelse(grepl("sera", asy), "(RIS-seraAS). ", ifelse(grepl("nasal", asy), "(RIS-nasalAS). ", ifelse(grepl("saliva", asy), "(RIS-salivaAS). ", "(RIS-PBMCAS). ")))
                        cohort_name <- ifelse(grepl("initial", tp), gsub("\\(", "(Track A ", cohort_name), cohort_name)
...
for (bsero in c(bstatus.labels.2, if(study_name=="VAT08") c("bstatus"#, "bstatus_gender"
    ...
    for(tp in times_[!times_ %in% c(paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
        ...
        for (pn in if (study_name != "VaxArt_Mock") {""} else {c(
          if (nextgen_output_flag >= 1) "IgG_sera",
          if (nextgen_output_flag >= 2) "IgA_sera", 
          if (nextgen_output_flag >= 2) "pseudoneutid50_sera",
          if (nextgen_output_flag == 3) "IgG_nasal", 
          if (nextgen_output_flag == 3) "IgA_nasal",
          if (nextgen_output_flag == 3) "pseudoneutid50_nasal", 
          if (nextgen_output_flag == 3) "IgG_saliva",
          if (nextgen_output_flag == 3) "IgA_saliva",
          if (nextgen_output_flag == 3) "pseudoneutid50_saliva",
          if (nextgen_output_flag >= 2) "T4_T8")}) {
          
          if (study_name == "VaxArt_Mock" & nextgen_output_flag == 1 & !tp %in% c("B", "Day31")) next
...
```{r, echo=FALSE, results='asis', eval=study_name %in% c("VAT08")}

for (tp in if (study_name == "VaxArt_Mock") {tps_no_fold_change} else {tps_no_delta_over_tinterm}){
    for (a in if (study_name == "VaxArt_Mock") {c(
      "IgG_sera", 
      "pseudoneutid50_sera", 
      if (nextgen_output_flag == 3) c("IgG_nasal", "pseudoneutid50_nasal", "IgG_saliva", "pseudoneutid50_saliva"), 
      "T4", "T8")
        ...
        #immuno_postfix_label <- case_when(
        #    study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366") ~ " (Track A RIS-PBMCAS cohort)",
        #    study_name == "VaxArt_Mock" & grepl("bind|pseudo", asy) ~ " (RIS cohort)",
        #    study_name == "VaxArt_Mock" & asy == "tcell" ~ " (RIS-PBMCAS cohort)",
        #    TRUE ~ immuno_postfix_label) 
        ...
        cohort_name <- ifelse(grepl("sera", a), "(RIS-seraAS). ", ifelse(grepl("nasal", a), "(RIS-nasalAS). ", ifelse(grepl("saliva", asy), "(RIS-salivaAS). ", "(RIS-PBMCAS). ")))
...
```{r, echo=FALSE, results='asis', eval=study_name %in% c("VAT08", "VaxArt_Mock")}

for (assay_grp in if (study_name == "VaxArt_Mock") {
    gsub("\\|", "__", gsub("\\$", "", c(
      "bindSpike_IgG_sera$|bindSpike_IgG_sera_delta_AY.4$",
     if (nextgen_output_flag >= 2) c("bindSpike_IgG_sera_BA.5$|bindSpike_IgG_sera_BA.2.86$",
     ...
     if (nextgen_output_flag == 3) c("bindSpike_IgG_nasal$|bindSpike_IgG_nasal_delta_AY.4$",
       ...
       if (nextgen_output_flag >= 2) "T4_IFNg_OR_IL2_N_Index$",
       
       "T4_IFNg_OR_IL2_Spike_KP.2$|T4_IFNg_OR_IL2_Spike_Index.D614$",
       
       if (nextgen_output_flag >= 2) "T8_IFNg_OR_IL2_N_Index$",
       
       "T8_IFNg_OR_IL2_Spike_KP.2$|T8_IFNg_OR_IL2_Spike_Index.D614$")))
} else {c("bAb","bAb_mdw","nAb","nAb_mdw")}){
    details = ""
    details_lb = ""
    #immuno_postfix_label <- ifelse(study_name == "VaxArt_Mock", " (RIS or RIS-PBMCAS cohort at Day01, Day31, Day181, Track A RIS-PBMCAS cohort at Day91, Day366)", immuno_postfix_label)
    
    assay_grp_lb = sub("^(.*?nAb ID50 )(.*?)(nAb ID50 )", "\\1\\2", # delete the second nab ID50
                       paste0(gsub("^Serum |^ |Serum IgG Spike |Serum IgA Spike | \\(AU/ml\\)|\\(AU/ml\\)", "", gsub("Serum IgG Spike D614 ", "bAb-IgG Spike Index", labels.assays.short[strsplit(assay_grp, "__")[[1]]])), collapse = " and ")
    )
    ...
    cohort_name <- ifelse(grepl("sera", assay_grp), "(RIS-seraAS). ", ifelse(grepl("nasal", assay_grp), "(RIS-nasalAS). ", ifelse(grepl("saliva", assay_grp), "(RIS-salivaAS). ", "(RIS-PBMCAS). ")))
    cohort_name <- gsub("\\(", "(Track A ", cohort_name)
...
for (bsero in c("non_naive")){
    ...
    for (trt in c("vac_pla", if (study_name == "VaxArt_Mock") "vac", if (study_name == "VaxArt_Mock") "pla")){
        
        for (tp in tps_no_fold_change){
          
            if (study_name == "VaxArt_Mock" & nextgen_output_flag == 1 & (!tp %in% c("B", "Day31") | trt == "pla")) next
            
            if (study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366")) next # skip for now
            
            if (study_name == "VaxArt_Mock" & trt == "vac_pla" & tp != "B") next # VaxArt_Mock has pooled outputs at baseline and separate outputs post baseline
            if (study_name == "VaxArt_Mock" & trt %in% c("vac", "pla") & tp == "B") next # VaxArt_Mock has pooled outputs at baseline and separate outputs post baseline
            ...
            for (asy in if (study_name != "VaxArt_Mock") {""} else {
                c("IgG_sera", 
                  ...
                  if (nextgen_output_flag >= 2) c("bindSpike_IgA_sera", "pseudoneutid50_sera"), 
                  
                  if (nextgen_output_flag == 3) c("IgG_nasal", "bindSpike_IgA_nasal", "pseudoneutid50_nasal", "IgG_saliva", "bindSpike_IgA_saliva", "pseudoneutid50_saliva"),
                  
                  if (nextgen_output_flag >= 2) c("T4", "T8", 
                  ...
                  if (nextgen_output_flag == 3) c("IgG_nasal_KP.2|bindSpike_IgA_nasal_KP.2", "IgG_nasal_KP.2|pseudoneutid50_nasal", "IgG_nasal_KP.2|T4", "IgG_nasal_KP.2|T8",
                  ...
                  if (nextgen_output_flag >= 2) "T4|T8")
                }) {
                assay_n = length(assays)

                asy_lb = case_when(asy == "IgG_sera" ~ "bAb-IgG Spike and bAb-IgG Nucleocapsid concentrations",
                                   asy == "IgG_nasal" ~ "bAb-IgG Spike and bAb-IgG Nucleocapsid concentrations",
                                   asy == "IgG_saliva" ~ "bAb-IgG Spike and bAb-IgG Nucleocapsid concentrations",
                                   
                                   asy == "bindSpike_IgA_sera" ~ "bAb-IgA Spike concentrations",
                                   asy == "bindSpike_IgA_nasal" ~ "bAb-IgA Spike concentrations",
                                   asy == "bindSpike_IgA_saliva" ~ "bAb-IgA Spike concentrations",
                                   
                                   asy == "pseudoneutid50_sera" ~ "nAb-ID50 titers",
                                   asy == "pseudoneutid50_nasal" ~ "nAb-ID50 titers",
                                   asy == "pseudoneutid50_saliva" ~ "nAb-ID50 titers",
                                   
                                   asy == "T4" ~ "CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "T8" ~ "CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   
                                   asy == "IgG_sera_KP.2|bindSpike_IgA_sera_KP.2" ~ "bAb-IgG Spike KP.2 and bAb-IgA Spike KP.2 concentrations",
                                   asy == "IgG_nasal_KP.2|bindSpike_IgA_nasal_KP.2" ~ "bAb-IgG Spike KP.2 and bAb-IgA Spike KP.2 concentrations",
                                   asy == "IgG_saliva_KP.2|bindSpike_IgA_saliva_KP.2" ~ "bAb-IgG Spike KP.2 and bAb-IgA Spike KP.2 concentrations",
                                   
                                   asy == "IgG_sera_KP.2|pseudoneutid50_sera" ~ "bAb-IgG Spike KP.2 concentrations and nAb-ID50 titers",
                                   asy == "IgG_nasal_KP.2|pseudoneutid50_nasal" ~ "bAb-IgG Spike KP.2 concentrations and nAb-ID50 titers",
                                   asy == "IgG_saliva_KP.2|pseudoneutid50_saliva" ~ "bAb-IgG Spike KP.2 concentrations and nAb-ID50 titers",
                                   
                                   asy == "IgG_sera_KP.2|T4" ~ "bAb-IgG Spike KP.2 concentrations and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "IgG_nasal_KP.2|T4" ~ "bAb-IgG Spike KP.2 concentrations and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "IgG_saliva_KP.2|T4" ~ "bAb-IgG Spike KP.2 concentrations and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   
                                   asy == "IgG_sera_KP.2|T8" ~ "bAb-IgG Spike KP.2 concentrations and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "IgG_nasal_KP.2|T8" ~ "bAb-IgG Spike KP.2 concentrations and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "IgG_saliva_KP.2|T8" ~ "bAb-IgG Spike KP.2 concentrations and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   
                                   asy == "bindSpike_IgA_sera_KP.2|pseudoneutid50_sera" ~ "bAb-IgA Spike KP.2 concentrations and nAb-ID50 titers",
                                   asy == "bindSpike_IgA_nasal_KP.2|pseudoneutid50_nasal" ~ "bAb-IgA Spike KP.2 concentrations and nAb-ID50 titers",
                                   asy == "bindSpike_IgA_saliva_KP.2|pseudoneutid50_saliva" ~ "bAb-IgA Spike KP.2 concentrations and nAb-ID50 titers",
                                   
                                   asy == "bindSpike_IgA_sera_KP.2|T4" ~ "bAb-IgA Spike KP.2 concentrations and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "bindSpike_IgA_nasal_KP.2|T4" ~ "bAb-IgA Spike KP.2 concentrations and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "bindSpike_IgA_saliva_KP.2|T4" ~ "bAb-IgA Spike KP.2 concentrations and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   
                                   asy == "bindSpike_IgA_sera_KP.2|T8" ~ "bAb-IgA Spike KP.2 concentrations and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "bindSpike_IgA_nasal_KP.2|T8" ~ "bAb-IgA Spike KP.2 concentrations and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "bindSpike_IgA_saliva_KP.2|T8" ~ "bAb-IgA Spike KP.2 concentrations and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   
                                   asy == "pseudoneutid50_sera|T4" ~ "nAb-ID50 titers and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "pseudoneutid50_nasal|T4" ~ "nAb-ID50 titers and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "pseudoneutid50_saliva|T4" ~ "nAb-ID50 titers and CD4+ T-cell markers to Spike and Nucleocapsid peptides",
                                   
                                   asy == "pseudoneutid50_sera|T8" ~ "nAb-ID50 titers and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "pseudoneutid50_nasal|T8" ~ "nAb-ID50 titers and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   asy == "pseudoneutid50_saliva|T8" ~ "nAb-ID50 titers and CD8+ T-cell markers to Spike and Nucleocapsid peptides",
                                   
                                   asy == "T4|T8" ~ "CD4+ and CD8+ T-cell markers to Spike and Nucleocapsid peptides")
                
                #immuno_postfix_label <- case_when(
                #    study_name == "VaxArt_Mock" & tp %in% c("B", "Day31", "Day181") ~ " (RIS or RIS-PBMCAS cohort)",
                #    study_name == "VaxArt_Mock" & tp %in% c("Day91", "Day366") ~ " (Track A RIS-PBMCAS cohort)",
                #    TRUE ~ immuno_postfix_label)
                specimen_type <- paste0("based on ", ifelse(grepl("sera", asy) & !grepl("T4|T8", asy), "sera", ifelse(grepl("nasal", asy) & !grepl("T4|T8", asy), "nasal fluid", ifelse(grepl("saliva", asy) & !grepl("T4|T8", asy), "saliva", ifelse(grepl("T4|T8", asy) & !grepl("sera|nasal|saliva", asy), "PBMC", "")))))
                specimen_type <- ifelse(specimen_type == "based on ", paste0("based on ", str_extract(asy, "sera|saliva|nasal"), " and PBMC"), specimen_type)
                cohort_name <- ifelse(grepl("sera", asy) & !grepl("T4|T8", asy), "(RIS-seraAS). ", ifelse(grepl("nasal", asy) & !grepl("T4|T8", asy), "(RIS-nasalAS). ", ifelse(grepl("saliva", asy) & !grepl("T4|T8", asy), "(RIS-salivaAS). ", ifelse(grepl("T4|T8", asy) & !grepl("sera|nasal|saliva", asy), "(RIS-PBMCAS). ", paste0("(RIS-", str_extract(asy, "sera|saliva|nasal"), ", RIS-PBMCAS). ") ))))
...
for (bsero in c("naive")){
    if (study_name == "VaxArt_Mock") next # VaxArt_Mock doesn't have naive ppts
...
for (cohort in if (study_name != "VaxArt_Mock") {""} else {c("_initial", "_final")}){
   
    for (a in assays){
      
    if (study_name == "VaxArt_Mock" & nextgen_output_flag == 1 & a != "pseudoneutid50_sera_KP.2") next
    #immuno_postfix_label <- case_when(
    #    study_name == "VaxArt_Mock" & grepl("bind|pseudo", a) ~ " (RIS cohort)",
    #    study_name == "VaxArt_Mock" & grepl("T4|T8", a) ~ " (RIS-PBMCAS cohort)",
    #    TRUE ~ immuno_postfix_label)
    specimen_type <- paste0("based on ", ifelse(grepl("sera", a), "sera", ifelse(grepl("nasal", a), "nasal fluid", ifelse(grepl("saliva", a), "saliva", "PBMC"))))
    cohort_name <- ifelse(grepl("sera", a), "(RIS-seraAS). ", ifelse(grepl("nasal", a), "(RIS-nasalAS). ", ifelse(grepl("saliva", a), "(RIS-salivaAS). ", "(RIS-PBMCAS). ")))
    cohort_name <- ifelse(grepl("initial", cohort), gsub("\\(", "(Track A ", cohort_name), cohort_name)
