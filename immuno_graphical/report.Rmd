    ...
    for (country in c("Nvx_US_Mex", if(attr(config,"config")=="prevent19") "Nvx_US")){
      ...
      for (asy in assay_immuno){
        ...
        for (times_index in c("", if (study_name == "NextGen_Mock") "2")){
          ...
          immuno_postfix_label <- ifelse(study_name == "NextGen_Mock" & times_index == "", "(Final Immunogenicity)",
                                         ifelse(study_name == "NextGen_Mock" & times_index == "2", "(Final Immunogenicity)",
    ...
    for(tp in times_[!times_ %in% c("B",paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
      ...
      for (asy in if(study_name=="VAT08" | attr(config,"config")=="janssen_partA_VL") {c("bAb","nAb")
        ...
        for (reg in if (attr(config,"config")=="janssen_partA_VL") {c("LATAM_","ZA_")} else {""}) {
          ...
          immuno_postfix_label <- ifelse(study_name == "NextGen_Mock" & tp %in% c("B", "Day31", "Day181"), "(Final Immunogenicity)",
                                         ifelse(study_name == "NextGen_Mock" & tp %in% c("Day91", "Day366"), "(Final Immunogenicity)",
                                                immuno_postfix_label))      
