...
for (bsero in bstatus.labels.2) {
  ...
  for (trt in c("vaccine","placebo", if(study_name=="VAT08") "pooled")) {  
    ...
    if (attr(config,"config")=="janssen_partA_VL" && !(trt=="vaccine" && bsero==bstatus.labels.2[1])) next # only report for vaccine baseline neg for janssan_partA_VL
    ...
    for (country in c("Nvx_US_Mex", if(study_name=="PREVENT19") "Nvx_US")){
      ...
      for (asy in assay_immuno){
        ...
        if (attr(config,"config")=="janssen_partA_VL") next # no figure for janssen_partA_VL
...
for (trt in c("vaccine_arm","placebo_arm")) {
  ...
  if (attr(config,"config")=="janssen_partA_VL") # janssen_partA_VL do not report this
...
```{r, fig.cap = paste0("Spaghetti plots of Ab markers over time: baseline negative vaccine + placebo arm"), eval = any(c("B", paste0("Day",timepoints[1]), "Day57") %in% times & study_name!="VAT08" & study_name!="ENSEMBLE")}
...
```{r, fig.cap = paste0("Spaghetti plots of Ab markers over time: baseline positive vaccine + placebo arm"), eval = any(c("B", paste0("Day",timepoints[1]), "Day57") %in% times & if.pos.exist==1 & study_name!="VAT08" & study_name!="ENSEMBLE")}
include_graphics(here(
  "immuno_graphical", "output", attr(config,"config"),
