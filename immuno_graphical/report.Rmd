...
for (bsero in bstatus.labels.2) {
    ...
    for (trt in c(trt.labels.2[2:1], if(study_name=="VAT08") "pooled")) {  
        ...
        for (country in c("Nvx_US_Mex", if(attr(config,"config")=="prevent19") "Nvx_US")){
            ...
            if (study_name == "NextGen_Mock" & nextgen_output_flag == 1) {
              assay_immuno_ = "pseudoneutid50_sera_XBB.1.5"
              
            } else {assay_immuno_ = assay_immuno}
          
            if (study_name == "NextGen_Mock" & nextgen_output_flag == 1 & trt == "comparator_vaccine") next
...
if (attr(config,"config") %in% c("vat08_combined","prevent19_stage2")){
 ...
 for(tp in if (study_name == "NextGen_Mock") {#c(paste0(tps_no_fold_change, "_initial"), paste0(tps_no_fold_change[c(1,2,4)], "_final"))
   c("_initial", "_final")} else {times_[!times_ %in% c("B",paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]}) {
    ...
    for (trt in c("trt_both", paste0("trt_", trt.labels.2[2]), if (study_name %in% c("VAT08")) paste0("trt_", trt.labels.2[1]))){
        ...
        for (bsero in c("bstatus_both", "bstatus_Neg", "bstatus_Pos")){
            ...
            if ((bsero != "bstatus_Pos"| trt != "trt_both") & study_name == "NextGen_Mock") next # NextGen_Mock only have bstatus_Pos, trt_both plot
            if (trt=="trt_both" && bsero!="bstatus_both" && !study_name %in% c("VAT08", "NextGen_Mock")) next # trt_both is only for bstatus both if the study is VAT08
...
```{r, echo=FALSE, results='asis', eval = study_name!="NextGen_Mock"}
...
if(study_name %in% c("VAT08", "NextGen_Mock") | attr(config,"config")=="janssen_partA_VL"){
    ...
    for (asy in if(study_name == "NextGen_Mock") {
      ...
      if (nextgen_output_flag >= 2) "T8")
        ...
        for (tp in #if(study_name=="VAT08") {c("Day", "Delta")} else {
            ...
            for (bsero in bstatus.labels.2) {
                ...
                for (trt in if (study_name == "NextGen_Mock") "trt_comparison" else {trt.labels.2[2:1]}) {
...
for (bsero in c(bstatus.labels.2, if(study_name=="VAT08") c("bstatus"#, "bstatus_gender"
    ...
    for(tp in times_[!times_ %in% c(paste0("Delta",timepoints_[length(timepoints_)],"over",timepoints_[1]))]) {
        ...
        for (pn in if (study_name != "NextGen_Mock") {""} else {c(
          ...
          if (study_name == "NextGen_Mock" & nextgen_output_flag == 1 & !tp %in% c("B", "Day31")) next
...
for (tp in if (study_name == "NextGen_Mock") {tps_no_fold_change} else {tps_no_delta_over_tinterm}){
    for (a in if (study_name == "NextGen_Mock") {c(
      "IgG_sera", 
      "pseudoneutid50_sera", 
...
for (bsero in c("non_naive")){
    ...
    for (trt in c("vac_pla", if (study_name == "NextGen_Mock") "vac", if (study_name == "NextGen_Mock") "pla")){
        ...
        for (tp in tps_no_fold_change){
            ...
            if (study_name == "NextGen_Mock" & nextgen_output_flag == 1 & (!tp %in% c("B", "Day31") | trt == "pla")) next
...
for (cohort in if (study_name != "NextGen_Mock") {""} else {c("_initial", "_final")}){
   
    for (a in assays){
      
    if (study_name == "NextGen_Mock" & nextgen_output_flag == 1 & a != "pseudoneutid50_sera_XBB.1.5") next
    #immuno_postfix_label <- case_when(
