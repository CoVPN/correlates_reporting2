  ...
  for (trt in c("vaccine","placebo", if(study_name=="VAT08") "pooled")) {  
    ...
    for (country in c("Nvx_US_Mex", if(attr(config,"config")=="prevent19") "Nvx_US")){
      ...
      for (asy in assay_immuno){
        ...
        for (times_index in c("", if (study_name == "NextGen_Mock") "2")){
          ...
          immuno_postfix_label <- ifelse(study_name == "NextGen_Mock" & times_index == "", "(Whole RIS/RIS-PBMC)",
                                         ifelse(study_name == "NextGen_Mock" & times_index == "2", "(Track A RIS/RIS-PBMC)",
...
for (trt in c("trt_both", "trt_vaccine", if (study_name %in% c("VAT08")) "trt_placebo")){
...
for (assay_grp in if (study_name == "NextGen_Mock") {
  c(paste(set2_assays[!grepl("mdw", set2_assays)][c(TRUE, FALSE)], 
          set2_assays[!grepl("mdw", set2_assays)][c(FALSE, TRUE)], sep = "_"))
...
for (bsero in c("naive")){
  if (study_name == "NextGen_Mock") # NextGen_Mock doesn't have naive ppts
