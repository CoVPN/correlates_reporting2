...
if (TRIAL=="vat08_combined") {
  ...
  if (Sys.getenv("stage")=="1") {
    ...
    res = knitr::knit_child(c(here::here('./report_trial', paste0('report_vat08_combined_1.Rmd'))), quiet = TRUE)
    cat(res, sep = '\n')
  } else if (Sys.getenv("stage")=="2") {
    message ("vat08_combined stage 2")
    res = knitr::knit_child(c(here::here('./report_trial', paste0('report_vat08_combined_2.Rmd'))), quiet = TRUE)
    cat(res, sep = '\n')
  } else {
...
} else if(file.exists(here::here('./report_trial', paste0('report_', TRIAL, '.Rmd')))
  ...
  res = knitr::knit_child(c(here::here('./report_trial', paste0('report_', TRIAL, '.Rmd'))), quiet = TRUE)
  cat(res, sep = '\n')

} else if(file.exists(here::here('./report_trial', paste0('report_', tolower(study_name), '.Rmd')))
   & attr(config,"config")!="profiscov_lvmn" # profiscov uses special template but lvmn does not because it does not have D43start48
) {

  message ("study name-specific report rmd found")
  res = knitr::knit_child(c(here::here('./report_trial', paste0('report_', tolower(study_name), '.Rmd'))), quiet = TRUE)
  cat(res, sep = '\n')
  ...
  for (tpeak in tt) {
    ...
    res = knitr::knit_child(c(here::here('.', 'report_by_COR.Rmd')), quiet = TRUE)
    cat(res, sep = '\n')
  }
