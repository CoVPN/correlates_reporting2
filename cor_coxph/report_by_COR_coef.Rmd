...
\normalsize
$\ast\ast$ No. at-risk = cohort size; no. cases = number of cases within this cohort. Both are ph1 numbers. Case indicator: `r EventIndPrimary`

`r if (!show.q) '<!--'`
`r tmp = paste0(pathCoRoutput, "/permutation_replicates_", fname.suffix)`
...
\\caption{Forest plots depicting hazard ratios per 10-fold change in marker, x-axis on the antilog scale. Case indicator: ", escape(EventIndPrimary), "}
\\end{figure}
", sep="")
...
\\caption{Forest plots depicting hazard ratios per 10-fold change in marker, x-axis on the log scale. Case indicator: ", escape(EventIndPrimary), "}
\\end{figure}
", sep="")
