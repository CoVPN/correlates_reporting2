...
if(!file.exists(paste0(save.results.to, "marginalized.risk.no.marker.Rdata"))) {    
    ...
    for (.trt in 0:1) {
        ...
        prob=get.marginalized.risk.no.marker(if (COR %in% c("D29SevereIncludeNotMolecConfirmed","D29VL")) form.0.list else form.0, dat.tmp, tfinal.tpeak)# competing risk estimation for severe cases            
        ...
        out=mclapply(1:B, mc.cores = 1, FUN=function(seed) {  
            ...
            get.marginalized.risk.no.marker(if (COR %in% c("D29SevereIncludeNotMolecConfirmed","D29VL")) form.0.list else form.0, dat.b, tfinal.tpeak)# competing risk estimation for severe cases            
