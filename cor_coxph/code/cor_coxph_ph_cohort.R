...
fits.cont.coef.ls = lapply(fits, function (fit) getFixedEf(fit))
...
fits.tri.coef.ls= lapply(fits.tri, function (fit) getFixedEf(fit))
...
natrisk = round(c(sapply (all.markers%.%"cat", function(a) aggregate(subset(dat,ph2==T)        [["wt"]], subset(dat,ph2==T        )[a], sum, na.rm=T, drop=F)[,2] )))
nevents = round(c(sapply (all.markers%.%"cat", function(a) aggregate(subset(dat,yy==1 & ph2)[["wt"]], subset(dat,yy==1 & ph2)[a], sum, na.rm=T, drop=F)[,2] )))
natrisk[is.na(natrisk)]=0
nevents[is.na(nevents)]=0
colSums(matrix(natrisk, nrow=3))
# regression parameters
est=c(rbind(1.00,  sapply(fits.tri, function (fit) if(length(fit)==1) rep(NA,2) else getFormattedSummary(list(fit), exp=T, robust=F, rows=rows, type=1))  ))
ci= c(rbind("N/A", sapply(fits.tri, function (fit) if(length(fit)==1) rep(NA,2) else getFormattedSummary(list(fit), exp=T, robust=F, rows=rows, type=13)) ))
p=  c(rbind("N/A", sapply(fits.tri, function (fit) if(length(fit)==1) rep(NA,2) else getFormattedSummary(list(fit), exp=T, robust=F, rows=rows, type=10)) ))
...
tmp=rbind(all.markers.names.short, "", ""); rownames(tab)=c(tmp)
tab
...
if (!is.null(config$multivariate_assays)) {
    ...
    for (a in config$multivariate_assays) {
      ...
      # 1: per SD; 2: per 10-fold
        ...
        fits=list(fit)
        est=getFormattedSummary(fits, exp=T, robust=F, rows=var.ind, type=1)
        ci= getFormattedSummary(fits, exp=T, robust=F, rows=var.ind, type=13)
        est = paste0(est, " ", ci)
        p=  getFormattedSummary(fits, exp=T, robust=F, rows=var.ind, type=10)
        ...
        mytex(tab, file.name=paste0("CoR_multivariable_coxph_pretty", match(a, config$multivariate_assays), if(i==2) "_per10fold", fname.suffix), align="c", include.colnames = T, save2input.only=T, 
...
if (!is.null(config$additional_models)) {
    ...
    for (a in config$additional_models) {
        ...
        fits=list(fit)
        est=getFormattedSummary(fits, exp=T, robust=F, type=1)
        ci= getFormattedSummary(fits, exp=T, robust=F, type=13)
        est = paste0(est, " ", ci)
        p=  getFormattedSummary(fits, exp=T, robust=F, type=10)
...
if (!is.null(config$interaction)) {
    ...
    for (ab in config$interaction) {
        ...
        fit=coxph(f, design=twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~ph2, data=dat)) 
        fits=list(fit)
        est=getFormattedSummary(fits, exp=T, robust=F, type=1)
        ci= getFormattedSummary(fits, exp=T, robust=F, type=13)
        est = paste0(est, " ", ci)
