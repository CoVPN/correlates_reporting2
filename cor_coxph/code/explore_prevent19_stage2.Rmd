...
# subset to US
dat_mapped = subset(dat_mapped, Country==0)
...
About half of the ptids have missing BD1 date, and many of the ptids with missing BD1 date have markers.
```{r}
mytable(dat_mapped$NumberdaysD1toBD1 - dat_mapped$DeltaEventTimeD1 < 0)
with(subset(dat_mapped, ph1.D35==1 & Country==0), mytable(is.na(NumberdaysD1toBD1), !is.na(Day35bindSpike_Delta3)))
subset(dat_mapped, ph1.D35==1 & Country==0 & is.na(NumberdaysD1toBD1) & !is.na(Day35bindSpike_Delta3), c(Subjectid, NumberdaysD1toC1, NumberdaysD1toBD1, DateDD1 ))[1:10,]
```


There are 13 severe cases in the study window, of which 7 have markers. The number is too small to study severe correlates.
```{r}
with(, mytable(Severity_first, !is.na(Day35bindSpike_Delta3))) #  & NumberdaysD1toBD1>=DeltaEventTimeD1
```
