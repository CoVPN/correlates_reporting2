...
dat_mapped=read.csv('/trials/covpn/p3004/analysis/mapping_immune_correlates/stage2/adata/COVID_Novavax_stage2_mapped_20240314.csv')
...
dat_proc=read.csv('/trials/covpn/p3004/analysis/correlates/Part_A_Blinded_Phase_Data/adata/prevent19_stage2_data_processed_20240315.csv')
...
dat_proc$VARTYPE_second1 = sapply(strsplit(dat_proc$VARTYPE_second,","), function (x) ifelse(length(x)==0, "Unknown", x[1]))
dat_proc$VARTYPE_second1 = ifelse(dat_proc$EventIndFirstD1==0, NA, dat_proc$VARTYPE_second1)
dat_proc$VARTYPE_second1 = ifelse(dat_proc$EventIndFirstD1==1 & is.na(dat_proc$VARTYPE_second1), "Unk", dat_proc$VARTYPE_second1)
...
There are 9 severe cases with markers. There are four degrees of severity. **Less-than-mild cases are not counted cases or censoring events.**
```{r}
with(subset(dat_proc, ph1.D35==1 & Country==0), mytable(Severity_first, ph2.D35, EventIndFirstD1))
```
...
Comparing timing of events of different variants, first using time since D35, second using calendar time. Ptids with markers are colored green. Based on the first plot, we define the followup period to start at **101** days post the D35 visit. Based on the second plot, we replace censoring at the data cut date of 2022-03-26 by censoring at **2021-12-10** to avoid the Omicron wave at the end of 2021 and the beginning of 2022.

```{r}
dat.tmp = subset(dat_proc, ph2.D35==1 & Country==0 & Trt==1)
col=ifelse(dat.tmp$Severity_first=="SEVERE", 2, ifelse(!is.na(dat.tmp$Day35bindSpike_Delta3),3,1))

myboxplot(EventTimeFirstD35~VARTYPE_first1, dat.tmp, ylab="EventTimeFirstD35", col=col)
abline(h=100, lty=2)

myboxplot(EventCalTimeFirstD35~VARTYPE_first1, dat.tmp, ylab="Event Calendar Time", col=col)
abline(h=as.Date("2022-03-26")) # data cut
abline(h=as.Date("2021-04-19")) # data cut
abline(h=as.Date("2021-12-10"), lty=2) 

with(subset(dat.tmp, Severity_first=="SEVERE"), mytable(VARTYPE_first1, ph2.D35))
```
...
names(dat)=sub("Day35pseudoneut","",names(dat))
names(dat)=sub("Day35bindSpike_","",names(dat))
mypairs(dat)
```
