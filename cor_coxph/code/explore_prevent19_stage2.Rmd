...
dat_mapped=read.csv('/trials/covpn/p3004/analysis/mapping_immune_correlates/stage2/adata/COVID_Novavax_stage2_mapped_20240311.csv')
...
This dataset uses the updated clinical database, which is slightly different from the Stage 1 dataset. The dataset contains vaccine arm only and seroneg only, but has both US and MEX.
```{r, include=F}
...
To study Delta cases, we 
```{r, include=T}
stopifnot(all(dat_mapped$DeltaEventTimeD35==dat_mapped$EventTimeFirstD35))
subset(dat_mapped, DeltaEventTimeD35!=EventTimeFirstD35, c(VARTYPE_first, VARTYPE_second, DeltaEventTimeD35, EventTimeFirstD35, EventTimeSecondD35))[1:10,]
```
...
```{r}
subset(dat_mapped, ph1.D35==1 & Country==0 & EventIndFirstD1==1 & is.na(NumberdaysD1toBD1) & !is.na(Day35bindSpike_Delta3), c(Subjectid, NumberdaysD1toC1, NumberdaysD1toBD1, DateDD1 ))[1:10,]
```



There are 16 severe cases in the study window, of which 9 have markers. The number is not sufficient to study severe correlates.
```{r}
...
Correlation between ancestral and Delta ID50s is high at each timepoint, and highest at BD1 (0.97) and DD1 (0.98).
```{r}
