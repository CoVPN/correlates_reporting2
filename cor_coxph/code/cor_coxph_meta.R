...
for (a in c("bindSpike")) {        
 mypdf(onefile=F, file=paste0("output/", a, "_controlled_ve_curves_cove_ensemble"), mfrow=.mfrow, oma=c(0,0,0,0))
    par(las=1, cex.axis=0.9, cex.lab=1)# axis label orientation
    
    
    ## get xlim.ls and source _commom.R
    xlim.ls=list()
    for (i in 1:2) {        
    ...
    # depends on lloxs, labels.assays, draw.x.axis.cor
    for (i in 1:2) {
...
#        config.cor <- config::get(config = COR)
        ...
        mymatplot(risks$marker, t(rbind(est, ci.band)), type="l", lty=c(1,2,2), lwd=2.5, make.legend=F, 
            ylab=paste0("Controlled VE"), 
