...
coxph(update(f, ~.  + Day15pseudoneutid50_D614G), subset(dat.1.moderna, Trt==0))
...
dat.ocp=subset(dat_proc, ph1.D15==1 & treatment_assigned %in% c(
    "Wildtype/Prototype (Pfizer 1)"
  , "Omicron (Pfizer 1)"
  , "Omicron + Wildtype/Prototype (Pfizer 1)"
  # , "Omicron BA.4/5 + Prototype (Pfizer 2)"
...
dat.ocp$Trt=ifelse(dat.ocp$treatment_assigned == "Wildtype/Prototype (Pfizer 1)", 0, 1)

coxph(update(f, ~. + naive + Trt), dat.ocp)
coxph(update(f, ~. + naive + Trt), subset(dat.ocp, naive==0))
coxph(update(f, ~. + naive + Trt), subset(dat.ocp, naive==1))

dat.1=subset(dat.ocp, naive==1)
```
