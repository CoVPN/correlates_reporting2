...
dat_mapped=read.csv('/trials/covpn/p3005/analysis/mapping_immune_correlates/combined/adata/COVID_Sanofi_stage1and2_mapped_20240612_hotdeck.csv')
dat_proc = read.csv('/trials/covpn/p3005/analysis/correlates/Part_A_Blinded_Phase_Data/adata/vat08_combined_data_processed_20240612.csv')
...
stage1step1=read.csv("~/JinStage1Step1.csv")
stage1step2=read.csv("~/JinStage1Step2.csv")
stage2700=read.csv("~/Aiying700.csv") # many rows for many markers
stage2step2=read.csv("/trials/covpn/p3005/analysis/correlates/Part_A_Blinded_Phase_Data/qdata/nab/Stage2CorrelatesListsandData/immunosubset.csv")
stage2cases=read.csv("/trials/covpn/p3005/analysis/correlates/Part_A_Blinded_Phase_Data/qdata/nab/Stage2CorrelatesListsandData/confirmed_cases_list.csv")

stage2step2$SUBJID = sub("VAT00008-","",stage2step2$USUBJID)
stage2step2$SUBJID = gsub("-","",stage2step2$SUBJID,)
stage2cases$SUBJID = sub("VAT00008-","",stage2cases$USUBJID)
stage2cases$SUBJID = gsub("-","",stage2cases$SUBJID,)


dat_mapped$stage1step1 = dat_mapped$SUBJID %in% stage1step1$SUBJID
dat_mapped$stage1step2 = dat_mapped$SUBJID %in% stage1step2$SUBJID

dat_proc$stage1step1 = dat_proc$SUBJID %in% stage1step1$SUBJID
dat_proc$stage1step2 = dat_proc$SUBJID %in% stage1step2$SUBJID
dat_proc$stage1step = NA
dat_proc$stage1step[dat_proc$stage1step1] = 1
dat_proc$stage1step[dat_proc$stage1step2] = 2


dat_mapped$stage2step2 = dat_mapped$Subjectid %in% stage2step2$USUBJID
dat_mapped$SubcohortInd[dat_mapped$Subjectid %in% stage2cases$USUBJID] = 0
dat_mapped$stage2step = NA
dat_mapped$stage2step[dat_mapped$SubcohortInd==1 & dat_mapped$Trialstage==2] = 1
dat_mapped$stage2step[dat_mapped$stage2step2 & dat_mapped$Trialstage==2] = 2


dat_proc$stage2step2 = dat_proc$SUBJID %in% stage2step2$SUBJID
dat_proc$SubcohortInd[dat_proc$SUBJID %in% stage2cases$SUBJID] = 0
dat_proc$stage2step = NA
dat_proc$stage2step[dat_proc$SubcohortInd==1 & dat_proc$Trialstage==2] = 1
dat_proc$stage2step[dat_proc$stage2step2 & dat_proc$Trialstage==2] = 2
mytable(dat_proc$stage2step)
...
```{r}
mytable(tmp$stage1step, tmp$nAbBatch)
```
...
mytable(tmp$SubcohortInd, tmp$stage1step2)
...
# lots of samples are neither cases  nor in the subcohort
with(subset(tmp, nAbBatch==1 | nAbBatch==2), mytable(EventIndFirstInfectionD1, SubcohortInd))
```
...
mytable(!is.na(tmp$Day43pseudoneutid50_B.1.351), !is.na(tmp$Day43pseudoneutid50), tmp$SubcohortInd)
mytable(!is.na(tmp$Day43pseudoneutid50_B.1.351), !is.na(tmp$Day43pseudoneutid50), tmp$nAbBatch)
mytable(!is.na(tmp$Day43pseudoneutid50_B.1.351)[tmp$SubcohortInd==1], !is.na(tmp$Day43pseudoneutid50)[tmp$SubcohortInd==1], tmp$nAbBatch[tmp$SubcohortInd==1])


mytable(tmp$EventIndFirstInfectionD1, tmp$SubcohortInd)
table.prop(tmp$EventIndFirstInfectionD1, tmp$SubcohortInd)
fisher.test(mytable(tmp$EventIndFirstInfectionD1, tmp$SubcohortInd))

mytable(!is.na(tmp$Day43pseudoneutid50_B.1.351), tmp$nAbBatch)
mytable(!is.na(tmp$Day43pseudoneutid50), tmp$nAbBatch)

mytable(!is.na(tmp$Day43pseudoneutid50_B.1.351), tmp$SubcohortInd)
mytable(!is.na(tmp$Day43pseudoneutid50), tmp$SubcohortInd)
```
...
table.prop(tmp$EventIndFirstInfectionD1, tmp$SubcohortInd)
fisher.test(table(tmp$EventIndFirstInfectionD1, tmp$SubcohortInd))

table.prop(tmp$EventIndFirstInfectionD1[tmp$region==1], tmp$SubcohortInd[tmp$region==1])
table.prop(tmp$EventIndFirstInfectionD1[tmp$region!=1], tmp$SubcohortInd[tmp$region!=1])

mytable(tmp$SubcohortInd, !is.na(tmp$Day43pseudoneutid50_B.1.351), tmp$EventIndFirstInfectionD1)
```
...
```{r}
mytable(tmp$stage2step, tmp$nAbBatch, tmp$EventIndFirstInfectionD1)
# D22
tmp1=subset(dat_mapped, Trialstage==2 & Trt==1 & ph1.D22 & Bserostatus==1)
mytable(tmp1$stage2step, tmp1$nAbBatch, tmp1$EventIndFirstInfectionD1)
```
...
fisher.test(table(tmp$EventIndFirstInfectionD1, tmp$stage2step2))
```
...
```{r}
with(subset(dat_proc, Trialstage==1 & ph2.D43.nAb & Bserostatus==1 & Trt==1), mytable(nAbBatch, EventIndOmicronD43M12hotdeck1, stage1step2))
with(subset(dat_proc, Trialstage==1 & ph2.D43.nAb.st1.anc & Bserostatus==1 & Trt==1), mytable(nAbBatch, EventIndOmicronD43M12hotdeck1, stage1step2))

with(subset(dat_proc, Trialstage==1 & ph2.D43.nAb), table(stage1step2))

with(subset(dat_proc, Trialstage==1), mytable(ph2.D43.nAb.st1.anc, ph2.D43.nAb, nAbBatch))


with(subset(dat_proc, Trialstage==1 & stage1step2==F), mytable(ph2.D43.nAb.st1.anc, ph2.D43.nAb))

with(subset(dat_proc, Trialstage==1 & stage1step2==T), mytable(ph2.D43.nAb.st1.anc, ph2.D43.nAb))

with(subset(dat_proc, Trialstage==1 & EventIndOmicronD43M12hotdeck1==1), mytable(ph2.D43.nAb.st1.anc, ph2.D43.nAb))

with(subset(dat_proc, Trialstage==1 & EventIndOmicronD43M12hotdeck1==1), mytable(stage1step2, nAbBatch))

with(subset(dat_proc, Trialstage==1 & EventIndOmicronD43M12hotdeck1==0 & ph2.D43.nAb), mytable(stage1step2, nAbBatch))



with(subset(dat_proc, Trialstage==1 & (ph2.D43.nAb.st1.anc) & !(ph2.D43.nAb)), mytable(nAbBatch, stage1step2))


with(subset(dat_proc, Trialstage==1), mytable(ph2.D43.nAb, ph2.D43.nAb.st1.anc, stage1step2))

with(subset(dat_proc, Trialstage==1), mytable(ph2.D43.nAb, ph2.D43.nAb.st1.anc, nAbBatch))

with(subset(dat_proc, Trialstage==1), mytable(stage1step2, nAbBatch))
