...
dat_mapped=read.csv('/trials/covpn/p3005/analysis/mapping_immune_correlates/combined/adata/COVID_Sanofi_stage1and2_mapped_20240720_hotdeck.csv')
dat_proc = read.csv('/trials/covpn/p3005/analysis/correlates/Part_A_Blinded_Phase_Data/adata/vat08_combined_data_processed_20240723.csv')
...
```{r}
dat1=subset(dat_proc, Trialstage==1 & Bserostatus==1 & ph1.D43)
dat2=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43)

plot (density(dat1$Bpseudoneutid50[!is.na(dat1$Bpseudoneutid50)], weights=dat1$wt.D43.nAb[!is.na(dat1$Bpseudoneutid50)]))
lines(density(dat2$Bpseudoneutid50[!is.na(dat2$Bpseudoneutid50)], weights=dat2$wt.D43.nAb[!is.na(dat2$Bpseudoneutid50)]), col=2)

Hmisc::wtd.quantile(dat1$Bpseudoneutid50, weights=dat1$Bpseudoneutid50, probs=c(1/4,1/2,3/4))
Hmisc::wtd.quantile(dat2$Bpseudoneutid50, weights=dat2$Bpseudoneutid50, probs=c(1/4,1/2,3/4))

svycoxph(Surv(EventTimeOmicronD43M6hotdeck10,EventIndOmicronD43M6hotdeck10) ~ Bhigh, twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~ph2.D43.nAb, data=subset(dat_proc, Trialstage==1 & Bserostatus==1 & ph1.D43 & Trt==1)))
svycoxph(Surv(EventTimeOmicronD43M6hotdeck10,EventIndOmicronD43M6hotdeck10) ~ I(1-Bhigh), twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~ph2.D43.nAb, data=subset(dat_proc, Trialstage==1 & Bserostatus==1 & ph1.D43 & Trt==1)))
```
...
for (iReg in 1:3) {
  myprint(iReg)
  print(summary(with(subset(dat_proc, Trialstage==1 & Bserostatus==1 & ph1.D43 & Trt==0 & region==iReg+1 & EventIndOmicronD43M6hotdeck1==1), as.Date(EnrollmentDate)+EventTimeOmicronD1M6hotdeck1)))
  print(summary(with(subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43 & Trt==0 & region==iReg   & EventIndOmicronD43M6hotdeck1==1), as.Date(EnrollmentDate)+EventTimeOmicronD1M6hotdeck1)))
}
