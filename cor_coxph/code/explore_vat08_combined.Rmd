...
```{r a request from Peter}
# For Sanofi Stage 2 non-case placebo recipients, calculate the Day 43 sero-response frequency with definition of sero-response:

#If Day 01 titer < LOD, call sero-positive if Day 43 titer > 4-fold LOD.
#If Day 01 titer > LOD, call sero-positive if Day 43 titer > 4-fold D01 value.

for (trt in 0:1) {
  myprint(trt)
  dat=subset(dat_proc, Trialstage==2 & Trt==trt & Bserostatus==1 & ph2.D43.nAb); nrow(dat)
  
  for (a in nassays) {
    dat[[glue("pos_{a}")]] = dat[[glue("B{a}")]] < 1.4 & dat[[glue("Day43{a}")]] > log10(160) | # replace log10(160) with 1.4 to reproduce results from the original def
                             dat[[glue("B{a}")]] > 1.4 & dat[[glue("Day43{a}")]] > dat[[glue("B{a}")]] + log10(4)
  }
  
  sapply(nassays, function (a) {
    formatDouble(wtd.mean(dat[[glue("pos_{a}")]], weights=dat$wt.D43.nAb)*100, 1)
  })
  
  # by case status
  print(mysapply(c("pseudoneutid50", "pseudoneutid50_B.1.351", "pseudoneutid50_BA.1"), function (a) {
    c(
      with (dat[dat$EventIndOmicronD43M5hotdeck10==1,], formatDouble(wtd.mean(get(glue("pos_{a}")), weights=wt.D43.nAb)*100, 1)%.%"%"),
      with (dat[dat$EventIndOmicronD43M5hotdeck10==0,], formatDouble(wtd.mean(get(glue("pos_{a}")), weights=wt.D43.nAb)*100, 1)%.%"%")
    )
  }))
...
mytable(dat_proc$batch0, dat_proc$nAbBatch)
```


```{r weights distribution}
dat=subset(dat_proc, Trialstage==2 & ph2.D43.nAb & Bserostatus==1)
mytable(round(dat$wt.D43.nAb))
mytable(dat$Wstratum, round(dat$wt.D43.nAb))

dat=subset(dat_proc, Trialstage==2 & ph2.D43.bAb & Bserostatus==1)
mytable(round(dat$wt.D43.bAb))
mytable(dat$Wstratum, round(dat$wt.D43.bAb))

dat=subset(dat_proc, Trialstage==2 & SubcohortIndnAb & Bserostatus==1)
mytable(round(dat$wt.immuno.nAb))
mytable(dat$tps.stratum, round(dat$wt.immuno.nAb))

dat=subset(dat_proc, Trialstage==2 & SubcohortIndbAb & Bserostatus==1)
mytable(round(dat$wt.immuno.bAb))
mytable(dat$tps.stratum, round(dat$wt.immuno.bAb))


```

```{r understanding baseline immune history indicators}
mytable(dat_mapped$baseline_immune_history, dat_mapped$Bserostatus)
mytable(rowSums( subset(dat_mapped, select=c(D01_N_pos, D01_NAAT_pos, D22_N_pos, D22_NAAT_pos)), na.rm=T) > 0, dat_mapped$D01_S_pos_only_in_non_naive_group, dat_mapped$Bserostatus)
...
mytable(dat$EventIndOmicronD43M5hotdeck10, dat$baseline_immune_history, dat$Trt)
mytable(dat$EventIndOmicronD43M5hotdeck10, dat$baseline_immune_history!="baseline_naive", dat$Trt)

```

```{r baseline anti-S}

dat_mapped=read.csv('/trials/covpn/p3005/analysis/mapping_immune_correlates/combined/adata/COVID_Sanofi_stage1and2_mapped_20250212.csv')

dat=subset(dat_mapped, Trialstage==1)
mytable(dat$baseline_immune_history, dat$D01_S_pos)
mytable(dat$SARSD22, dat$SARSD01)
with(subset(dat, SARSD22=="Non-Naive-D01/D22" & SARSD01=="Naive-D01"), mytable(D22_N_pos, D22_NAAT_pos))
mytable(dat$SARSD22, dat$D01_S_pos==1 | dat$D01_N_pos==1 | dat$D01_NAAT_pos==1 | dat$D22_N_pos==1 | dat$D22_NAAT_pos==1)

with(subset(dat, SARSD22=="Non-Naive-D01/D22"), table(D01_S_pos_only_in_non_naive_group, !is.na(Day43bindSpike) | !is.na(Day43pseudoneutid50)))



dat=subset(dat_mapped, Trialstage==2)
mytable(dat$baseline_immune_history, dat$D01_S_pos)
mytable(dat$SARSD22, dat$D01_S_pos)
mytable(dat$SARSD22, dat$D01_S_pos_only_in_non_naive_group)
mytable(dat$SARSD22, dat$D01_S_pos==1 & dat$D01_N_pos==0 & dat$D01_NAAT_pos==0 & dat$D22_N_pos==0 & dat$D22_NAAT_pos==0)
mytable(dat$SARSD22, dat$D01_S_pos==1 | dat$D01_N_pos==1 | dat$D01_NAAT_pos==1 | dat$D22_N_pos==1 | dat$D22_NAAT_pos==1)

with(subset(dat, SARSD22=="Non-Naive-D01/D22"), mytable(D01_S_pos_only_in_non_naive_group, !is.na(Day43bindSpike) | !is.na(Day43pseudoneutid50)))
with(subset(dat, SARSD22=="Non-Naive-D01/D22"), mytable(D01_S_pos_only_in_non_naive_group, SamplingStep))
with(subset(dat, D01_S_pos_only_in_non_naive_group==1), mytable(!is.na(Day43bindSpike), !is.na(Day43pseudoneutid50), SamplingStep))
with(subset(dat, D01_S_pos_only_in_non_naive_group==1), mytable(nAbBatch, SamplingStep))
with(subset(dat, SARSD22=="Non-Naive-D01/D22"), mytable(nAbBatch, SamplingStep))

with(subset(dat, SARSD22=="Non-Naive-D01/D22"), table(D01_S_pos, !is.na(Day43bindSpike) | !is.na(Day43pseudoneutid50)))
with(subset(dat, SARSD22=="Non-Naive-D01/D22"), table(D01_N_pos, !is.na(Day43bindSpike) | !is.na(Day43pseudoneutid50)))
with(subset(dat, SARSD22=="Non-Naive-D01/D22"), table(D01_NAAT_pos, !is.na(Day43bindSpike) | !is.na(Day43pseudoneutid50)))

with(subset(dat), table(!is.na(Day43bindSpike) | !is.na(Day43pseudoneutid50), D01_S_pos_only_in_non_naive_group, Country))

mytable(dat$D01_S_pos, dat$D01_N_pos, dat$D01_NAAT_pos)

mytable(dat$EventIndFirstInfectionD1, dat$baseline_immune_history, dat$Trt)
...
```{r}
dat=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43)
table(!is.na(dat$BbindSpike), dat$ph2.D43.bAb)
```

For Stage 2 non-naïve ptids (pooling over vaccine and placebo), show something like the density of the calendar dates between 7 days post D43 and 150 days post D22.
```{r}
df <- subset(dat_proc, Trialstage==2 & Bserostatus==1  & ph1.D43) # EventTimeOmicronD22M5hotdeck10
summary(df$EnrollmentDate+df$EventTimeOmicronD22M5hotdeck10+df$NumberdaysD1toD22)
summary(df$EnrollmentDate)

par(mfrow=c(2,1), mar=c(2,4,2,1))

breaks <- as.Date(c("2021-12-01", "2022-01-01", "2022-02-01", "2022-03-01", "2022-04-01", "2022-05-01", "2022-06-01", "2022-07-01", "2022-08-01", "2022-09-01"))

hist( df$EnrollmentDate+df$EventTimeOmicronD22M5hotdeck10+df$NumberdaysD1toD22, breaks = breaks, col = "skyblue", border = "white", xlab = "", main = "Stage 2 NN, Loss to Followup Date", freq=T) # , xlim=as.Date(c("2021-12-01", "2022-07-01"))

hist((df$EnrollmentDate+df$EventTimeOmicronD22M5hotdeck10+df$NumberdaysD1toD22)[df$EventIndOmicronD22M5hotdeck10==1], breaks = breaks, col = "skyblue", border = "white", xlab = "", main = "Stage 2 NN Cases, Event Date", freq=T) # , xlim=as.Date(c("2021-12-01", "2022-07-01"))

# hist(df$EnrollmentDate, breaks = breaks, col = "skyblue", border = "white", ylab="Frequency of Enrollment Date", xlab = "", main = "Stage 2 NN Enrollment Date", freq=T)

```


Define a variable for a small project for Peter and Li
```{r}
tmp = subset(dat_proc, EventIndKnownLineageOmicronOrMissingLineageD22==1 & EventIndKnownLineageOmicronD22==0)
tmp1 = tmp[,c("EventTimeFirstInfectionD22", "EventTimeKnownLineageOmicronOrMissingLineageD22")]
tmp[tmp1[,1]!=tmp1[,2],c("EventTimeFirstInfectionD22", "EventTimeKnownLineageOmicronOrMissingLineageD22","EventTimeKnownLineageOmicronD22", "EventTimeOmicronD22M12hotdeck10")]


dat <- read.csv("/trials/covpn/p3005/analysis/correlates/Part_A_Blinded_Phase_Data/adata/vat08_combined_data_processed_20241119.csv")
df <- filter(dat, Trialstage==2 & Perprotocol==1 & EventTimeOmicronD22M12hotdeck10 >= 14 & !is.na(Bserostatus)  &
               EventTimeOmicronD22M12hotdeck10 <= 360 & !is.na(dat$seq1.variant.hotdeck10) & seq1.variant.hotdeck10=="Omicron")
a <- filter(df, df$EventIndFirstInfectionD22 == 1 & df$EventIndOmicronD22M12hotdeck10 == 0)
a$Ptid
...
fit coxph models with svydesign instead of twophase
```{r}
dat=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43 & Trt==1)

a=assays[1]
f=as.formula(paste0("Surv(EventTimeOmicronD43M5hotdeck10 ,EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + Day43",a," + strata(cc)"))

# using twophase
fit = svycoxph(f, twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~ph2.D43.bAb, data=dat)); fit

# using svydesign
fit1 = svycoxph(f, svydesign(id=~1, strata=~Wstratum, weights=~wt.D43.bAb, data=dat[dat$ph2.D43.bAb==1,])); fit1

# the comparison shows close alignment of two results
getFormattedSummary(list(fit,fit1), exp=T, robust=T)
```


Percent of participants eligible for D43 correlates analyses
```{r}
dat=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43 & ph2.D43.bAb)
weighted.mean(dat$stage2_D01_S_pos_only_in_non_naive_group, dat$wt.D43.bAb)

dat=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43 & ph2.D43.nAb)
weighted.mean(dat$stage2_D01_S_pos_only_in_non_naive_group, dat$wt.D43.nAb)

with(subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43), table(Trt, stage2_D01_S_pos_only_in_non_naive_group))
```


Run corcoxph in stage2_D01_S_pos_only_in_non_naive_group for vaccine arm
```{r}
dat=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43 & Trt==1 & stage2_D01_S_pos_only_in_non_naive_group==0)
mytable(dat$EventIndOmicronD43M5hotdeck10)

bAb.fits=lapply (assays[1:9], function(a) {
  f=as.formula(paste0("Surv(EventTimeOmicronD43M5hotdeck10 ,EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + Day43",a," + strata(cc)"))
  fit = svycoxph(f, svydesign(id=~1, strata=~Wstratum, weights=~wt.D43.bAb, data=dat[dat$ph2.D43.bAb==1,])); fit
})

tab.bAb=t(getFormattedSummary(bAb.fits, robust=T, exp=T, rows=4))
rownames(tab.bAb)=assays[1:9]
tab.bAb


nAb.fits=lapply (assays[10:15], function(a) {
  f=as.formula(paste0("Surv(EventTimeOmicronD43M5hotdeck10 ,EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + Day43",a," + strata(cc)"))
  fit = svycoxph(f, svydesign(id=~1, strata=~Wstratum, weights=~wt.D43.nAb, data=dat[dat$ph2.D43.nAb==1,])); fit
})

tab.nAb=t(getFormattedSummary(nAb.fits, robust=T, exp=T, rows=4))
rownames(tab.nAb)=assays[10:15]
tab.nAb
...
Run corcoxph in stage2_D01_S_pos_only_in_non_naive_group for placebo
```{r}
dat=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43 & Trt==0 & stage2_D01_S_pos_only_in_non_naive_group==0)
mytable(dat$EventIndOmicronD43M5hotdeck10)
...
stage 2 An interaction model with arm * marker
```{r}
dat=subset(dat_proc, Trialstage==2 & Bserostatus==1 & ph1.D43)

bAb.fits=lapply (assays[1:9], function(a) {
  f=as.formula(paste0("Surv(EventTimeOmicronD43M5hotdeck10 ,EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + Day43",a,"*Trt + strata(cc)"))
  fit = svycoxph(f, twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~ph2.D43.bAb, data=dat))
  fit
})

nAb.fits=lapply (assays[10:15], function(a) {
  f=as.formula(paste0("Surv(EventTimeOmicronD43M5hotdeck10 ,EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + Day43",a,"*Trt + strata(cc)"))
  fit = svycoxph(f, twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~ph2.D43.nAb, data=dat))
  fit
})
...
trial stage 1 Batch 0 assay dates
```{r}
tmp=subset(dat_proc, Trialstage==1 & Bserostatus==1 & Trt==1 & ph2.D43.st1.nAb.batch0and1)

library(readxl)

dates=as.data.frame(read_xlsx("~/VAT08_SCHARP Request_MGRM_updated_10182024.xlsx", sheet=2))

tmp$DATE_ASSAY = dates$DATE_ASSAY [match(tmp$SUBJID, dates$SUBJID)]
...
wtd.mean(dat2$Bpseudoneutid50>1.302, wt)
...
title(main="in Green: 12% ", line=-0.2)
...
title(main="in Green: 17%", line=-0.2)
...
tmp=subset(dat_proc, Trialstage==2 & Bserostatus==1 & Trt==1 & ph2.D43.bAb)
col=1
lim=c(2,6)
# bAb
par(mfrow=c(2,2), oma=c(0,0.5,3,0.5), mar=c(3.1, 3.1, 2.1, 0.1), mgp = c(2, 1, 0))
with(tmp, 
{
# D43
plot(BbindSpike, Day43bindSpike, xlim=lim, ylim=lim, main="Ancestral bAb", asp=1, col=ifelse(EventIndOmicronD43M5hotdeck10==1,2,1)); abline(0,1); abline(h=c(5.29,5.61))
plot(BbindSpike, Day43bindSpike_omicron, col=col, xlim=lim, ylim=lim, main="Omicron bAb", asp=1); abline(0,1)
# D22
plot(BbindSpike, Day22bindSpike, col=col, xlim=lim, ylim=lim, main="Ancestral bAb", asp=1); abline(0,1)
plot(BbindSpike, Day22bindSpike_omicron, col=col, xlim=lim, ylim=lim, main="Omicron bAb", asp=1); abline(0,1)
#
...
wtd.cor(x=tmp$BbindSpike, y=tmp$Day43bindSpike, weights = NULL)

```

Fit a model with both baseline and fold rise
```{r}
df=subset(dat_proc, Trialstage==2 & Bserostatus==1 & Trt==1 & ph1.D43)
design.vac<-twophase(id=list(~1,~1), strata=list(NULL,~Wstratum), subset=~ph2.D43.bAb, data=df)

fit=svycoxph(Surv(EventTimeOmicronD43M5hotdeck10, EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + strata(Country) + I(scale(Day43bindSpike,scale=FALSE))*I(scale(BbindSpike,scale=FALSE)), design=design.vac) 
fit

fit=svycoxph(Surv(EventTimeOmicronD43M5hotdeck10, EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + strata(Country) + I(scale(Day43bindSpike,scale=FALSE))+I(scale(BbindSpike,scale=FALSE)), design=design.vac) 
fit

fit=svycoxph(Surv(EventTimeOmicronD43M5hotdeck10, EventIndOmicronD43M5hotdeck10) ~ FOI + risk_score + Sex + strata(Country) + I(scale(BbindSpike,scale=FALSE)), design=design.vac) 
fit
```
...
col=1
# nAb
par(mfrow=c(2,2), oma=c(0,0.5,3,0.5), mar=c(2.1, 2.1, 2.1, 0.1), mgp = c(2, 1, 0))
lim=c(0,6)
