  ...
  for (tpeak in tt) { # 22 and 43
    ...
    j=2     # skip M12 analyses
      ...
      for (i in ii) {
        ...
        has.alt = F
        coef_page_notes = "dich(B) = 1 if baseline marker is high, 0 if baseline marker is low. The threshold is set to the weighted median unless the negative response rate at baseline exceeds 40% or more than 40% of the values are above the Upper Limit of Quantification (ULOQ). In these cases, the threshold is adjusted to the detection limit or ULOQ, respectively. The second parameter is the effect of the continuous marker in the low baseline group."
        res = knitr::knit_child(c(here::here('cor_coxph', 'report_by_COR_coef.Rmd')), quiet = TRUE)
        ...
        has.alt = F
        coef_page_notes = "dich(B) = 1 if baseline marker is high, 0 if baseline marker is low. The threshold is set to the weighted median unless the negative response rate at baseline exceeds 40% or more than 40% of the values are above the Upper Limit of Quantification (ULOQ). In these cases, the threshold is adjusted to the detection limit or ULOQ, respectively. The second parameter is the effect of the continuous marker in the high baseline group."
        res = knitr::knit_child(c(here::here('cor_coxph', 'report_by_COR_coef.Rmd')), quiet = TRUE)
        coef_page_notes = ""
        cat(res, sep = '\n')
