...
$\ast\ast\ast$q-value and FWER (family-wide error rate) are computed over the set of p-values both for quantitative markers and categorical markers `r if(study_name=="PREVENT19") "(not including categorical bindSpike due to there being no case in the upper tertile)"` using the Westfall and Young permutation method (\protect\input{`r pathCoRoutput`/permutation_replicates_`r fname.suffix`} replicates). 
`r special.note`
...
\input{`r pathCoRoutput`/marginalized_risks_cat_`r fname.suffix`}  \\ \hline 
\end{tabular}
\end{center}
