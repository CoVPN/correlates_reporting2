...
if (!TRIAL %in% c("nextgen_mock")) { # some trials are organized differently, e.g. may have tables from two treatment arms together
  res = knitr::knit_child(c(here::here('.', 'report_by_COR_coef.Rmd')), quiet = TRUE)
  cat(res, sep = '\n')
}
```
...
```{r, echo=FALSE, results='asis'}
if (show.ve.curves) {
  res = knitr::knit_child(c(here::here('.', 'report_by_COR_curves.Rmd')), quiet = TRUE)
  cat(res, sep = '\n')
}
```
...
if (TRIAL!='covail_frnt') { # skip trichotomized because hard to compare with id50 results
  ...
  res = knitr::knit_child(c(here::here('.', 'report_by_COR_tri.Rmd')), quiet = TRUE)
  cat(res, sep = '\n')
}
```
...
```{r, echo=FALSE, results='asis'}
  ...
  if (!is.null(tmp)) {
    ...
    for (i in 1:length(tmp)) {
      res = knitr::knit_child(c(here::here('.', 'multiple_regression.Rmd')), quiet = TRUE)
      cat(res, sep = '\n')
    }
  }
```
...
<!--       res = knitr::knit_child(c(here::here('.', 'additional_model.Rmd')), quiet = TRUE) -->
<!--       cat(res, sep = '\n') -->
<!--     } -->
<!--   } -->
<!-- ``` -->
...
<!--       a=tmp[1] -->
<!--       b=tmp[2] -->
<!--       res = knitr::knit_child(c(here::here('.', 'itxn_by_assays.Rmd')), quiet = TRUE) -->
<!--       cat(res, sep = '\n') -->
<!--   } -->
<!-- } -->
<!-- ``` -->
