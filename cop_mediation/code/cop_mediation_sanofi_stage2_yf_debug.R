# re-implementation to check my code
...

{
  ...
  dat.seropos.st2 = subset(dat_proc, kp==1 & Trialstage==2 & ph1)
  
  imp=10 # imputation copy to use
  
  # overwrite covariates to be consistent with Sydney's implementation
  config$covariates = "~standardized_risk_score+FOI"
  
  iSt=2 # stage
  if (iSt==2) {
    dat=dat.seropos.st2
    save.results.to=save.results.to%.%"stage2nnaive/"
    if (!dir.exists(save.results.to)) dir.create(save.results.to)
  }
...
for (marker_set in marker_sets) {
  ...
  results_list = lapply (all.markers, function(a) {
    ...
    fit_trt1_medtrt1 <- survtmle::hazard_tmle(
      ftime = dat[[config.cor$EventTimePrimary %.% imp]],
      ftype = dat[[config.cor$EventIndPrimary %.% imp]],
      trt = dat$Trt,
