...
message("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ running immuno_tabular report ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
```
...
dats <- list.files(here::here( "immuno_tabular", "output", attr(config,"config")))
```
\captionsetup[table]{}
...
numi <- 1
for (d in dats){
base::load(here::here("immuno_tabular", "output", attr(config,"config"), d))
  ...
  for (j in jloop) {
      ...
      caption.j <- ifelse(!is.na(loop), sprintf("%s (%s)", title_i, j), title_i)
