...
maketab=function(.PPAI, trt, digit) {
  tab = sapply(panels[1:4], function(p) {
        sapply(antigens, function(a) {
          ...
          out  = wilcox_test_quiet(dat.tmp, conf.int=TRUE)
          ci   = out$conf.int
          stars = ifelse(out$p.value < 0.001, "***", ifelse(out$p.value < 0.01, "**", ifelse(out$p.value < 0.05, "*", "")))
          glue("{formatDouble(10**(mean(dat.tmp)),digit)}{stars} ({formatDouble(10**(ci[1]),digit)}, {formatDouble(10**(ci[2]),digit)})")
        })
  })
...
# impossible to install packages to write tables to Word because R is too old
mywrite.csv(tab, file="Table1", row.names=T)
...
pc1_reg = function(endpoint, PPAI.only, trt) {
  tab = mysapply (c("B","Day28","Delta28overB"), function (t) {
    ...
    getFormattedSummary(fits, robust="MBN", exp=T, sig.level=0.1)[2,,drop=F]
  })
...
mywrite.csv(tab, row.names=T, file="Table_2_1")
...
mywrite.csv(tab, row.names=T, file="Table_S2_1")
...
kable(tab, align="c", digits=2, caption="PC1 foldrise median") %>% 
  column_spec(1:ncol(tab), extra_css = "padding-left:20px; padding-right:20px;") %>% 
  add_header_above(c(" "=1, "Placebo"=ncol(tab1), "BPZE1"=ncol(tab1)))

# save a version to csv
# remove B_Day28
tab=tab[,c(1,2,4,5,6,8)]
mywrite.csv(tab, row.names = T, file="tab_foldrise_median", digits = 2)
```
...
for (p in panels[1:4]){
  ...
  axis(1, at=c(1.5, 3.5), labels=c("Placebo", "BPZE1"), line=2, col="white")
