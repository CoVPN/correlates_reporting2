...
# ensemble.dat <- read.csv("/Volumes/trials/covpn/p3003/analysis/mapping_immune_correlates/post unblinded part B stage 2/adata/COVID_ENSEMBLE_stage2_mapped_20250929.csv")
# ensemble.dat <- read.csv("/Volumes/trials/covpn/p3003/analysis/mapping_immune_correlates/post unblinded part B stage 2/adata/COVID_ENSEMBLE_stage2_mapped_20251030.csv")
if(!exists("spath")) {
...
ensemble.dat$AdjAsymEventIndPrimaryBD14_CensorAtOutsideVacc_ME <- 
  ifelse(ensemble.dat$AdjAsymEventIndPrimaryBD14_CensorAtOutsideVacc == 1 &
         ensemble.dat$AdjModtoSevEventIndPrimaryBD14_CensorAtOutsideVacc == 1,
         0, ensemble.dat$AdjAsymEventIndPrimaryBD14_CensorAtOutsideVacc)
ensemble.dat$AdjMildEventIndPrimaryBD14_CensorAtOutsideVacc_ME <- 
  ifelse((ensemble.dat$AdjAsymEventIndPrimaryBD14_CensorAtOutsideVacc_ME == 1 |
         ensemble.dat$AdjModtoSevEventIndPrimaryBD14_CensorAtOutsideVacc == 1) &
         ensemble.dat$AdjAnyEventIndPrimaryBD14_CensorAtOutsideVacc == 1,
...
write.csv(Tab, paste0(spath, "/reports/AdjEndpoints_", Sys.Date(), ".csv"))
# write.csv(Tab, "../reports/AdjEndpoints.csv")

### median follow-up
median(ensemble.dat$AdjAnyEventTimePrimaryBD14_CensorAtOutsideVacc)
median(ensemble.dat$AdjAnyEventTimePrimaryBD14_CensorAtOutsideVacc[ensemble.dat$AdjAnyEventIndPrimaryBD14_CensorAtOutsideVacc == 0])
max(ensemble.dat$AdjAnyEventTimePrimaryBD14_CensorAtOutsideVacc)

median(ensemble.dat$AdjAnyEventTimePrimaryBD14_CensorAtOutsideVacc)
max(ensemble.dat$AdjAnyEventTimePrimaryBD14_CensorAtOutsideVacc)

median(ensemble.dat$AdjSevEventTimePrimaryBD14_CensorAtOutsideVacc)
max(ensemble.dat$AdjSevEventTimePrimaryBD14_CensorAtOutsideVacc)


###############################################################
### Event counts and event rate by group, restricted to 90 days
################################################################
...
# write.csv(Tab, "/Volumes/ahudson/Ensemble-stage2-correlates/SummaryTables/AdjEndpoints90Days.csv")
write.csv(Tab, paste0(spath, "/reports/AdjEndpoints90Days_", Sys.Date(), ".csv"))
...
pdf(paste0(spath, "/reports/AdjSurvivalCurves_", Sys.Date(), ".pdf"))
...
fit <- survfit(Surv(event = AdjSevEventIndPrimaryBD14_CensorAtOutsideVacc, time = AdjSevEventTimePrimaryBD14_CensorAtOutsideVacc) ~ group,
               data = ensemble.dat)
ggsurvplot(fit, data = ensemble.dat, risk.table = TRUE, censor = FALSE, title = "Severe COVID", fun = "event",
           conf.int = TRUE, xlim = c(0, 210))
...
fit <- survfit(Surv(time = AdjAnyEventTimePrimaryBD14_CensorAtOutsideVacc, event = AdjAnyEventIndPrimaryBD14_CensorAtOutsideVacc) ~ group,
               data = ensemble.dat)
ggsurvplot(fit, data = ensemble.dat, risk.table = TRUE, censor = FALSE, title = "Infection, Any Symptomatology", fun = "event",
           conf.int = TRUE, xlim = c(0, 210))
dev.off()

pdf(paste0(spath, "/reports/AdjSurvivalCurves90Days_", Sys.Date(), ".pdf"))
fit <- survfit(Surv(event = AdjModtoSevEventIndPrimaryBD14_CensorAtOutsideVacc, time = AdjModtoSevEventTimePrimaryBD14_CensorAtOutsideVacc) ~ group,
               data = ensemble.dat)
ggsurvplot(fit, data = ensemble.dat, risk.table = TRUE, censor = FALSE, title = "Primary COVID", fun = "event",
...
# ensemble.dat$group %in% c("Delayed Hybrid", "Delayed Vaccine")

# with(ensemble.dat, head(TRTSDT[group == "Immediate Hybrid"]))
# with(ensemble.dat, head(TRTSDT[group == "Delayed Hybrid"]))

c1 <- rgb(173,216,230,max = 255, alpha = 80, names = "lt.blue")
c2 <- rgb(255,192,203, max = 255, alpha = 80, names = "lt.pink")
c3 <- rgb(235,192,52,max = 255, alpha = 80, names = "lt.gold")
c4 <- rgb(91,235,52, max = 255, alpha = 80, names = "lt.pink")

pdf(paste0(spath, "/reports/DateDose2_", Sys.Date(), ".pdf"))
