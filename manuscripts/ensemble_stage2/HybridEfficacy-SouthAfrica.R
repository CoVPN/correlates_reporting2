...
fit.sev <- CumulativeIncidenceFit(event = event.sev[complete], time = time.sev[complete],
                                  A = A[complete], W = W.sev[complete,],
                                  t0 = 90, fit.times = seq(1, 90, 1), np = FALSE)
...
out.sev <- CumulativeIncidenceSummary(time = time.sev[complete], event = event.sev[complete],
                                     W = W.sev[complete,],
                                     A = A[complete], t0 = max(time.sev[event.sev == 1]), fit = fit.sev,
                                     t.start = 1, label = "Severe-critical (South Africa)",
                                     time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                     path.prefix = paste0(spath, "/reports/"),
                                     path.suffix = paste0("severe-RSA_", Sys.Date()), y.axis.bound.risk = .25,
                                     time.axis.offset = 13)
out.any.mod <- CumulativeIncidenceSummary(time = time.any.mod[complete], event = event.any.mod[complete],
                                          W = W[complete,],
                                          A = A[complete], t0 = max(time.any.mod[event.any.mod == 1]), fit = fit.any.mod,
                                          t.start = 1, label = "Moderate to Severe-critical (South Africa)",
                                          time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                          path.prefix = paste0(spath, "/reports/"),
                                          path.suffix = paste0("moderate-to-severe-RSA_", Sys.Date()), y.axis.bound.risk = .25,
                                          time.axis.offset = 13)
out.any <- CumulativeIncidenceSummary(time = time.any[complete], event = event.any[complete],
                                      W = W[complete,],
                                      A = A[complete], t0 = t0, fit = fit.any,
                                      t.start = 1, label = "Infection, Any Symptomatology (South Africa)",
                                      time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                      path.prefix = paste0(spath, "/reports/"),
                                      path.suffix = paste0("any-infection-RSA_", Sys.Date()), y.axis.bound.risk = .25,
                                      time.axis.offset = 13)
...
# write.csv(tab, "/Volumes/ahudson/Ensemble-stage2-correlates/EfficacyImmunoCorrelates/Efficacy-RSA-1-22-2026.csv")
write.csv(tab, paste0(spath, "/reports/Efficacy-RSA_", Sys.Date(), ".csv"))