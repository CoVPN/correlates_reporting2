...
##B.1
tab.hy <- matrix(NA, nrow = 3, ncol = 2)
tab.vac <- matrix(NA, nrow = 3, ncol= 2)
tab.int <- matrix(NA, nrow = 3, ncol = 2)
...
cal.time.num <- as.numeric(as.Date(ensemble.dat.immuno$SVSTDTC_YEAR.1_28.DAYS))

complete <- apply(cbind(A, W, X), 1, function(x){all(!is.na(x))}) & ensemble.dat.immuno$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc > 0

cox.sev.hy <- coxph(Surv(cal.time.num[A == 1 & complete], 
                         cal.time.num[A == 1 & complete] + ensemble.dat.immuno$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 1 & complete],
                         ensemble.dat.immuno$AdjSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 1 & complete]) ~
                      X[A == 1 & complete] + W[A == 1 & complete,], weights = ensemble.dat.immuno$wt.immuno[A == 1 & complete],
                    robust = TRUE, id = 1:sum(A == 1 & complete))
cox.sev.vac <- coxph(Surv(cal.time.num[A == 0 & complete], 
                          cal.time.num[A == 0 & complete] +  ensemble.dat.immuno$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 0 & complete],
                          event = ensemble.dat.immuno$AdjSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 0 & complete]) ~
                       X[A == 0 & complete] + W[A == 0 & complete,], weights = ensemble.dat.immuno$wt.immuno[A == 0 & complete],
                     robust = TRUE, id = 1:sum(A == 0 & complete))
cox.sev.int <- coxph(Surv(cal.time.num[complete], 
                          cal.time.num[complete] +  ensemble.dat.immuno$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc[complete],
                          event = ensemble.dat.immuno$AdjSevEventIndPrimaryBD35_CensorAtOutsideVacc[complete]) ~
                       X[complete] * A[complete] + W[complete,], weights = ensemble.dat.immuno$wt.immuno[complete],
                     robust = TRUE,  id = 1:sum(complete))

complete <- apply(cbind(A, W, X), 1, function(x){all(!is.na(x))}) & ensemble.dat.immuno$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc > 0

cox.modtosev.hy <- coxph(Surv(cal.time.num[A == 1 & complete], 
                              cal.time.num[A == 1 & complete] + ensemble.dat.immuno$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 1 & complete],
                              event = ensemble.dat.immuno$AdjModtoSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 1 & complete]) ~
                           X[A == 1 & complete] + W[A == 1 & complete,], weights = ensemble.dat.immuno$wt.immuno[A == 1 & complete],
                         robust = TRUE,  id = 1:sum(A == 1 & complete))
cox.modtosev.vac <- coxph(Surv(cal.time.num[A == 0 & complete], 
                               cal.time.num[A == 0 & complete] + ensemble.dat.immuno$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 0 & complete],
                               event = ensemble.dat.immuno$AdjModtoSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 0 & complete]) ~
                            X[A == 0 & complete] + W[A == 0 & complete,], weights = ensemble.dat.immuno$wt.immuno[A == 0 & complete],
                          robust = TRUE,  id = 1:sum(A == 0 & complete))
cox.modtosev.int <- coxph(Surv(cal.time.num[complete], 
                               cal.time.num[complete] + ensemble.dat.immuno$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc[complete],
                               event = ensemble.dat.immuno$AdjModtoSevEventIndPrimaryBD35_CensorAtOutsideVacc[complete]) ~
                            X[complete] * A[complete] + W[complete,], weights = ensemble.dat.immuno$wt.immuno[complete],
                          robust = TRUE,  id = 1:sum(complete))

complete <- apply(cbind(A, W, X), 1, function(x){all(!is.na(x))}) & ensemble.dat.immuno$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc > 0

cox.any.hy <- coxph(Surv(cal.time.num[A == 1 & complete], 
                         cal.time.num[A == 1 & complete] + ensemble.dat.immuno$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc[A == 1 & complete],
                         event = ensemble.dat.immuno$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc[A == 1 & complete]) ~
                      X[A == 1 & complete] + W[A == 1 & complete,], weights = ensemble.dat.immuno$wt.immuno[A == 1 & complete],
                    robust = TRUE,  id = 1:sum(A == 1 & complete))
cox.any.vac <- coxph(Surv(cal.time.num[A == 0 & complete], 
                          cal.time.num[A == 0 & complete] + ensemble.dat.immuno$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc[A == 0 & complete],
                          event = ensemble.dat.immuno$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc[A == 0 & complete]) ~
                       X[A == 0 & complete] + W[A == 0 & complete,], weights = ensemble.dat.immuno$wt.immuno[A == 0 & complete],
                     robust = TRUE,  id = 1:sum(A == 0 & complete))
cox.any.int <- coxph(Surv(cal.time.num[complete], 
                          cal.time.num[complete] + ensemble.dat.immuno$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc[complete],
                          event = ensemble.dat.immuno$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc[complete]) ~
                       X[complete] * A[complete] + W[complete,], weights = ensemble.dat.immuno$wt.immuno[complete],
                     robust = TRUE,  id = 1:sum(complete))


tab.hy[1,1] <- paste0(round(summary(cox.sev.hy)$coefficients[1,2], 2), " (",
                      round(exp(summary(cox.sev.hy)$coefficients[1,1] - 
                                  qnorm(.975) *summary(cox.sev.hy)$coefficients[1,4]), 2),
                      ", ",
                      round(exp(summary(cox.sev.hy)$coefficients[1,1] +
                                  qnorm(.975) *summary(cox.sev.hy)$coefficients[1,4]), 2), ")")
tab.hy[1,2] <- signif(summary(cox.sev.hy)$coefficients[1,6])

tab.hy[2,1] <- paste0(round(summary(cox.modtosev.hy)$coefficients[1,2], 2), " (",
                      round(exp(summary(cox.modtosev.hy)$coefficients[1,1] - 
                                  qnorm(.975) *summary(cox.modtosev.hy)$coefficients[1,4]), 2),
                      ", ",
                      round(exp(summary(cox.modtosev.hy)$coefficients[1,1] +
                                  qnorm(.975) *summary(cox.modtosev.hy)$coefficients[1,4]), 2), ")")
tab.hy[2,2] <- signif(summary(cox.modtosev.hy)$coefficients[1,6])

tab.hy[3,1] <- paste0(round(summary(cox.any.hy)$coefficients[1,2], 2), " (",
                      round(exp(summary(cox.any.hy)$coefficients[1,1] - 
                                  qnorm(.975) *summary(cox.any.hy)$coefficients[1,4]), 2),
                      ", ",
                      round(exp(summary(cox.any.hy)$coefficients[1,1] +
                                  qnorm(.975) *summary(cox.any.hy)$coefficients[1,4]), 2), ")")
tab.hy[3,2] <- signif(summary(cox.any.hy)$coefficients[1,6])

tab.vac[1,1] <- paste0(round(summary(cox.sev.vac)$coefficients[1,2], 2), " (",
                       round(exp(summary(cox.sev.vac)$coefficients[1,1] - 
                                   qnorm(.975) *summary(cox.sev.vac)$coefficients[1,4]), 2),
                       ", ",
                       round(exp(summary(cox.sev.vac)$coefficients[1,1] +
                                   qnorm(.975) *summary(cox.sev.vac)$coefficients[1,4]), 2), ")")
tab.vac[1,2] <- signif(summary(cox.sev.vac)$coefficients[1,6])

tab.vac[2,1] <- paste0(round(summary(cox.modtosev.vac)$coefficients[1,2], 2), " (",
                       round(exp(summary(cox.modtosev.vac)$coefficients[1,1] - 
                                   qnorm(.975) *summary(cox.modtosev.vac)$coefficients[1,4]), 2),
                       ", ",
                       round(exp(summary(cox.modtosev.vac)$coefficients[1,1] +
                                   qnorm(.975) *summary(cox.modtosev.vac)$coefficients[1,4]), 2), ")")
tab.vac[2,2] <- signif(summary(cox.modtosev.vac)$coefficients[1,6])

tab.vac[3,1] <- paste0(round(summary(cox.any.vac)$coefficients[1,2], 2), " (",
                       round(exp(summary(cox.any.vac)$coefficients[1,1] - 
                                   qnorm(.975) *summary(cox.any.vac)$coefficients[1,4]), 2),
                       ", ",
                       round(exp(summary(cox.any.vac)$coefficients[1,1] +
                                   qnorm(.975) *summary(cox.any.vac)$coefficients[1,4]), 2), ")")
tab.vac[3,2] <- signif(summary(cox.any.vac)$coefficients[1,6])

tab.int[1,1] <- paste0(round(summary(cox.sev.int)$coefficients[7,2], 2), " (",
                       round(exp(summary(cox.sev.int)$coefficients[7,1] - 
                                   qnorm(.975) *summary(cox.sev.int)$coefficients[7,4]), 2),
                       ", ",
                       round(exp(summary(cox.sev.int)$coefficients[7,1] +
                                   qnorm(.975) *summary(cox.sev.int)$coefficients[7,4]), 2), ")")
tab.int[1,2] <- signif(summary(cox.sev.int)$coefficients[7,6])

tab.int[2,1] <- paste0(round(summary(cox.modtosev.int)$coefficients[7,2], 2), " (",
                       round(exp(summary(cox.modtosev.int)$coefficients[7,1] - 
                                   qnorm(.975) *summary(cox.modtosev.int)$coefficients[7,4]), 2),
                       ", ",
                       round(exp(summary(cox.modtosev.int)$coefficients[7,1] +
                                   qnorm(.975) *summary(cox.modtosev.int)$coefficients[7,4]), 2), ")")
tab.int[2,2] <- signif(summary(cox.modtosev.int)$coefficients[7,6])

tab.int[3,1] <- paste0(round(summary(cox.any.int)$coefficients[7,2], 2), " (",
                       round(exp(summary(cox.any.int)$coefficients[7,1] - 
                                   qnorm(.975) *summary(cox.any.int)$coefficients[7,4]), 2),
                       ", ",
                       round(exp(summary(cox.any.int)$coefficients[7,1] +
                                   qnorm(.975) *summary(cox.any.int)$coefficients[7,4]), 2), ")")
tab.int[3,2] <- signif(summary(cox.any.int)$coefficients[7,6])


tab <- cbind(tab.hy, tab.vac, tab.int)
rownames(tab) <- c("Severe-critical", "Mod to Severe-critical", "Any Infection")
colnames(tab) <- c("Hybrid (HR, CI)", "Hybrid (P-value)", 
                   "Vaccine (HR, CI)", "Vaccine (P-value)",
                   "Interaction HR (CI)", "Interaction HR (P-value)")
# write.csv(tab, "/Volumes/ahudson/Ensemble-stage2-correlates/EfficacyImmunoCorrelates/CoxCorrelates-B1.csv")
write.csv(tab, paste0(spath, "/reports/CoxCorrelates-B1_", Sys.Date(), ".csv"))
...
# complete <- complete & ((ensemble.dat.immuno$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc == 1 &
#                            ensemble.dat.immuno$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc <= 90) |
#                           (ensemble.dat.immuno$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc == 0 &
#                              ensemble.dat.immuno$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc > 90))
...
colnames(tab) <- c("Hybrid (HR, CI)", "Hybrid (P-value)", 
                   "Vaccine (HR, CI)", "Vaccine (P-value)",
                   "Interaction HR (CI)", "Interaction HR (P-value)")
# write.csv(tab, "/Volumes/ahudson/Ensemble-stage2-correlates/EfficacyImmunoCorrelates/CoxCorrelates-BA45.csv")
write.csv(tab, paste0(spath, "/reports/CoxCorrelates-BA45_", Sys.Date(), ".csv"))
