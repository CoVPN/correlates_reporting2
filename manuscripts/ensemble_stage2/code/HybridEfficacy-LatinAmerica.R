...
out.sev <- CumulativeIncidenceSummary(time = time.sev[complete], event = event.sev[complete],
                                     ...
                                     t.start = 1, label = "Severe-critical (Latin America)",
                                     time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                     path.prefix = paste0(spath, "/reports/"),
                                     path.suffix = paste0("severe-LA_", Sys.Date()), y.axis.bound.risk = .35,
                                     time.axis.offset = 13)
out.any.mod <- CumulativeIncidenceSummary(time = time.any.mod[complete], event = event.any.mod[complete],
                                          ...
                                          t.start = 1, label = "Moderate to Severe-critical (Latin America)",
                                          time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                          path.prefix = paste0(spath, "/reports/"),
                                          path.suffix = paste0("moderate-to-severe-LA_", Sys.Date()), y.axis.bound.risk = .35,
                                          time.axis.offset = 13)
out.any <- CumulativeIncidenceSummary(time = time.any[complete], event = event.any[complete],
                                      W = W[complete,],
                                      A = A[complete], t0 = t0, fit = fit.any,
                                      t.start = 1, label = "Infection, Any Symptomatology (Latin America)",
                                      time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                      path.prefix = paste0(spath, "/reports/"),
                                      path.suffix = paste0("any-infection-LA_", Sys.Date()), y.axis.bound.risk = .35,
                                      time.axis.offset = 13)
...
#                                             t.start = 1, label = "Severe-critical (Latin America)",
#                                             time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
#                                             path.suffix = "severe-LatinAmerica-adap-axis-1-22-2026",
#                                             time.axis.offset = 13)
...
#                                                 t.start = 1, label = "Moderate to Severe-critical (Latin America)",
#                                                 time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
#                                                 path.suffix = "moderate-to-severe-LatinAmerica-adap-axis-1-22-2026",
#                                                 time.axis.offset = 13)
...
#                                             t.start = 1, label = "Infection, Any Symptomatology (Latin America)",
#                                             time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
#                                             path.suffix = "any-infection-LatinAmerica-adap-axis-1-22-2026",
#                                             time.axis.offset = 13)
...
# write.csv(tab, "/Volumes/ahudson/Ensemble-stage2-correlates/EfficacyImmunoCorrelates/Efficacy-LatinAmerica-1-22-2026.csv")
write.csv(tab, paste0(spath, "/reports/Efficacy-LatinAmerica_", Sys.Date(), ".csv"))
