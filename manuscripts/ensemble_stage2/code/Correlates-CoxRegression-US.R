...
X <-  subset(subset(ensemble.dat.immuno, Region == 0))$Year1_28dayspseudoneutid50_B.1
A <- with(subset(ensemble.dat.immuno, Region == 0), ifelse(group %in% c("Immediate Hybrid", "Delayed Hybrid"), 1, 0))
W <- with(subset(ensemble.dat.immuno, Region == 0), cbind(Age, Sex, BMIDich * Sex, BMIDich * (1 - Sex)))
cal.time.num <- as.numeric(as.Date(subset(ensemble.dat.immuno, Region == 0)$SVSTDTC_YEAR.1_28.DAYS))

# complete <- apply(cbind(A, W, X), 1, function(x){all(!is.na(x))}) & subset(ensemble.dat.immuno, Region == 0)$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc > 0
# 
# cox.sev.hy <- coxph(Surv(cal.time.num[A == 1 & complete], 
#                          cal.time.num[A == 1 & complete] + subset(ensemble.dat.immuno, Region == 0)$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 1 & complete],
#                          subset(ensemble.dat.immuno, Region == 0)$AdjSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 1 & complete]) ~
#                       X[A == 1 & complete] + W[A == 1 & complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[A == 1 & complete],
#                     robust = TRUE, id = 1:sum(A == 1 & complete))
# cox.sev.vac <- coxph(Surv(cal.time.num[A == 0 & complete], 
#                           cal.time.num[A == 0 & complete] +  subset(ensemble.dat.immuno, Region == 0)$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 0 & complete],
#                           event = subset(ensemble.dat.immuno, Region == 0)$AdjSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 0 & complete]) ~
#                        X[A == 0 & complete] + W[A == 0 & complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[A == 0 & complete],
#                      robust = TRUE, id = 1:sum(A == 0 & complete))
# cox.sev.int <- coxph(Surv(cal.time.num[complete], 
#                           cal.time.num[complete] +  subset(ensemble.dat.immuno, Region == 0)$AdjSevEventTimePrimaryBD35_CensorAtOutsideVacc[complete],
#                           event = subset(ensemble.dat.immuno, Region == 0)$AdjSevEventIndPrimaryBD35_CensorAtOutsideVacc[complete]) ~
#                        X[complete] * A[complete] + W[complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[complete],
#                      robust = TRUE,  id = 1:sum(complete))

complete <- apply(cbind(A, W, X), 1, function(x){all(!is.na(x))}) & subset(ensemble.dat.immuno, Region == 0)$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc > 0

cox.modtosev.hy <- coxph(Surv(cal.time.num[A == 1 & complete], 
                              cal.time.num[A == 1 & complete] + subset(ensemble.dat.immuno, Region == 0)$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 1 & complete],
                              event = subset(ensemble.dat.immuno, Region == 0)$AdjModtoSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 1 & complete]) ~
                           X[A == 1 & complete] + W[A == 1 & complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[A == 1 & complete],
                         robust = TRUE,  id = 1:sum(A == 1 & complete))
cox.modtosev.vac <- coxph(Surv(cal.time.num[A == 0 & complete], 
                               cal.time.num[A == 0 & complete] + subset(ensemble.dat.immuno, Region == 0)$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc[A == 0 & complete],
                               event = subset(ensemble.dat.immuno, Region == 0)$AdjModtoSevEventIndPrimaryBD35_CensorAtOutsideVacc[A == 0 & complete]) ~
                            X[A == 0 & complete] + W[A == 0 & complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[A == 0 & complete],
                          robust = TRUE,  id = 1:sum(A == 0 & complete))
cox.modtosev.int <- coxph(Surv(cal.time.num[complete], 
                               cal.time.num[complete] + subset(ensemble.dat.immuno, Region == 0)$AdjModtoSevEventTimePrimaryBD35_CensorAtOutsideVacc[complete],
                               event = subset(ensemble.dat.immuno, Region == 0)$AdjModtoSevEventIndPrimaryBD35_CensorAtOutsideVacc[complete]) ~
                            X[complete] * A[complete] + W[complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[complete],
                          robust = TRUE,  id = 1:sum(complete))

complete <- apply(cbind(A, W, X), 1, function(x){all(!is.na(x))}) & subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc > 0

cox.any.hy <- coxph(Surv(cal.time.num[A == 1 & complete], 
                         cal.time.num[A == 1 & complete] + subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc[A == 1 & complete],
                         event = subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc[A == 1 & complete]) ~
                      X[A == 1 & complete] + W[A == 1 & complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[A == 1 & complete],
                    robust = TRUE,  id = 1:sum(A == 1 & complete))
cox.any.vac <- coxph(Surv(cal.time.num[A == 0 & complete], 
                          cal.time.num[A == 0 & complete] + subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc[A == 0 & complete],
                          event = subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc[A == 0 & complete]) ~
                       X[A == 0 & complete] + W[A == 0 & complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[A == 0 & complete],
                     robust = TRUE,  id = 1:sum(A == 0 & complete))
cox.any.int <- coxph(Surv(cal.time.num[complete], 
                          cal.time.num[complete] + subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc[complete],
                          event = subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc[complete]) ~
                       X[complete] * A[complete] + W[complete,], weights = subset(ensemble.dat.immuno, Region == 0)$wt.immuno[complete],
                     robust = TRUE,  id = 1:sum(complete))
...
# write.csv(tab, "/Volumes/ahudson/Ensemble-stage2-correlates/EfficacyImmunoCorrelates/CoxCorrelates-B1-US.csv")
write.csv(tab, paste0(spath, "/reports/CoxCorrelates-B1-US_", Sys.Date(), ".csv"))
...
# W <- with(subset(ensemble.dat.immuno, Region == 0), cbind(Age, Sex, BMIDich * Sex, BMIDich * (1 - Sex)))
W <- with(subset(ensemble.dat.immuno, Region == 0), cbind(Age, Sex, BMIDich * Sex, BMIDich * (1 - Sex)))
# W <- with(subset(ensemble.dat.immuno, Region == 0), cbind(Age, BMIDich * Sex, BMIDich * (1 - Sex),
#                                      RegionUS, RegionLA, RegionRSA, HistoryA, CalendarDateDose2Early))

# complete <- apply(cbind(A, W, X), 1, function(x){all(!is.na(x))})
# complete <- complete & ((subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc == 1 &
#                            subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc <= 90) |
#                           (subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventIndPrimaryBD35_CensorAtOutsideVacc == 0 &
#                              subset(ensemble.dat.immuno, Region == 0)$AdjAnyEventTimePrimaryBD35_CensorAtOutsideVacc > 90))
...
colnames(tab) <- c("Hybrid (HR, CI)", "Hybrid (P-value)", 
                   "Vaccine (HR, CI)", "Vaccine (P-value)",
                   "Interaction HR (CI)", "Interaction HR (P-value)")
# write.csv(tab, "/Volumes/ahudson/Ensemble-stage2-correlates/EfficacyImmunoCorrelates/CoxCorrelates-BA45-US.csv")
write.csv(tab, paste0(spath, "/reports/CoxCorrelates-BA45-US_", Sys.Date(), ".csv"))
