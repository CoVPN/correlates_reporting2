...
if (attr(config,"config")=="prevent19_stage2") {
  times = c("BD1","Day35","Delta35overBD1")
  time_labels = c("Booster Day 1","Day 35", "D35 fold-rise over BD1")
}
...
#} else {
  ...
  if (COR_postfix_list == "adhoc"){ # only for config: janssen_partA_VL and COR: D29VLvariant
    ...
    for (COR_postfix in COR_postfix_list){
      ...
      # SET 1
      for (a in c("pseudoneutid50", "bindSpike", if(attr(config,"config") %in% c("prevent19_stage2","azd1222_stage2")) "bindSpike_sub_nvx_stage2")){
      ...
      for (assay_grp in assay_grp_list){
        ...
        if ((grepl("nab", COR_postfix) & sum(grepl("pseudoneutid50", assay_grp)))==0 | (grepl("bab", COR_postfix) & sum(grepl("bindSpike", assay_grp)))==0 ) next # for AZ stage 2
      ...
      for (bsero in c("naive")){
                #delete############ below
        if (attr(config,"config") %in% c("azd1222_stage2")) next
          
          for (trt in c("vac")){
              
              for (tp in if (attr(config,"config")=="prevent19_stage2") {c("BD1","Day35","Delta35overBD1")} else {tps_no_B_and_fold_change}){
                  
                  tpLabel = ifelse(tp=="B", "Day1", tp)
                  assay_n = case_when(COR %in% c("D57azd1222_stage2_delta_nAb", "D57azd1222_stage2_severe_nAb") ~ 2,
                                      COR %in% c("D57azd1222_stage2_delta_bAb", "D57azd1222_stage2_severe_bAb") ~ 8,
                                      TRUE ~ length(assays))
                  res = knitr::knit_child(c(here::here('cor_graphical', 'report_set3.Rmd')), quiet = TRUE)
                  cat(res, sep = '\n')
                  cat("\\clearpage", sep = '\n')
