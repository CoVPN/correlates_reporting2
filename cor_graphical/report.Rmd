...
#} else {
  ...
  if (COR_postfix_list == "adhoc"){ # only for config: janssen_partA_VL and COR: D29VLvariant
    ...
    for (COR_postfix in COR_postfix_list){
      ...
      # SET 1
      for (a in if (study_name == "NextGen_Mock") {c(if (nextgen_output_flag == 1) assays[4], if (nextgen_output_flag == 3) assays[!grepl("IgA", assays)])
      ...
      assay_grp_list = if(attr(config,"config")=="vat08_combined"){
        c(paste0(assays[!assays=="bindSpike_mdw"][1:2], collapse="_"),
          ...
          c(if (nextgen_output_flag == 1) assays[4], if (nextgen_output_flag == 3) assays[!grepl("mdw", assays)])
        } else {assays}
