...
#} else {
  ...
  } else if (study_name == "VAT08" | attr(config,"config")=="prevent19_stage2"){
    
    for (COR_postfix in COR_postfix_list){
        
      # show in figure title
      COR_postfix_label = ""
      if (COR_postfix == "delta") COR_postfix_label=" (Delta Endpoint)"
      if (COR_postfix == "severe") COR_postfix_label=" (Severe Endpoint)"
      
      COR=ifelse(COR_postfix=="", "", 
                 ifelse(COR_postfix=="delta", "D35prevent19_stage2_delta", 
                         ifelse(COR_postfix=="severe", "D35prevent19_stage2_severe")))
      
      # SET 1
      for (a in c("pseudoneutid50", "bindSpike", if(attr(config,"config")=="prevent19_stage2") "bindSpike_sub_nvx_stage2")){
      ...
      assay_grp_list = if(attr(config,"config")=="vat08_combined"){
        ...
        )} else if (attr(config,"config")=="prevent19_stage2"){c("pseudoneutid50_D614G_pseudoneutid50_Delta","bindSpike_D614_bindSpike_Delta1")}
      ...
      for (bsero in c("non_naive")){
        if (attr(config,"config")=="prevent19_stage2") next # prevent19_stage2 doesn't have non_naive
