...
if (!(study_name %in% c("ENSEMBLE", "MockENSEMBLE", "janssen_partA_VL", "IARCHPV", "VAT08", "UK302", "COVAIL", "NextGen_Mock", "ILIAD_IB202P")) & !grepl("stage2|prevent19nvx", attr(config,"config"))) {
...
#} else {
  ...
  if (study_name=="ILIAD_IB202P") COR_postfix_list <- c("C0iliad_ib202p")
  ...
  if (COR_postfix_list == "adhoc"){ # only for config: janssen_partA_VL and COR: D29VLvariant
    ...
    for (COR_postfix in COR_postfix_list){
      ...
      # SET 1
cat("\\Large \\textbf{Violin plots by case status}\n\n")
cat("\\clearpage", sep = '\n')
      
      for (a in if (study_name == "ILIAD_IB202P") {assays} else if (study_name == "NextGen_Mock") {c(if (nextgen_output_flag == 1) assays[2], if (nextgen_output_flag == 3) assays[!grepl("IgA", assays)])
        ...
        for (tp in if (study_name %in% c("NextGen_Mock", "ILIAD_IB202P")) {tps_no_delta_over_tinterm_lb[1:3]} else {tps_no_delta_over_tinterm_lb}){
      ...
      # SET 2
cat("\\Large \\textbf{Longitudinal plots}\n\n")
cat("\\clearpage", sep = '\n')
      ...
      assay_grp_list = if(attr(config,"config")=="vat08_combined"){
        c(paste0(assays[!assays=="bindSpike_mdw"][1:2], collapse="_"),
          ...
          c(paste0(assays[1:2], collapse="_"), paste0(assays[3:4], collapse="_"),
            paste0(assays[5:6], collapse="_"), paste0(assays[7:8], collapse="_"),
            paste0(assays[9:10], collapse="_"), paste0(assays[11:12], collapse="_"),
            paste0(assays[13:14], collapse="_"), paste0(assays[15:16], collapse="_"),
            paste0(assays[17:18], collapse="_"), paste0(assays[19:20], collapse="_"),
            paste0(assays[21:22], collapse="_")
            )
        } else if (attr(config,"config")=="prevent19_stage2"){
      ...
      # SET 3
cat("\\Large \\textbf{Correlation plots across assays}\n\n")
cat("\\clearpage", sep = '\n')
      ...
      # SET 4
cat("\\Large \\textbf{Correlation plots across timepoints}\n\n")
cat("\\clearpage", sep = '\n')
