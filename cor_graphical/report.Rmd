...
if (!(study_name %in% c("ENSEMBLE", "MockENSEMBLE", "janssen_partA_VL", "IARCHPV"))) {
  plot_ve_curves <- readRDS(here::here("cor_graphical", "data_clean", "plot_ve_curves.rds"))
}
bsero <- ifelse(study_name=="vat08m_nonnaive", "BaselinePos", 
                ifelse(study_name=="IARCHPV", "", "BaselineNeg"))
  ...
  if (COR_postfix_list == "adhoc"){ # only for config: janssen_partA_VL and COR: D29variant
    ...
    for (COR_postfix in COR_postfix_list){
      ...
      if (study_name=="IARCHPV"){
        ...
        for (marker in c("BAb","NAb")){
          res = knitr::knit_child(c(here::here('cor_graphical', 'report_pair_variant_adhoc.Rmd')), quiet = TRUE)
          cat(res, sep = '\n')
          cat("\\clearpage", sep = '\n')
        }
        ############################################# Violin plots ###########################################
