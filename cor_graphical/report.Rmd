...
if (!(study_name %in% c("ENSEMBLE", "MockENSEMBLE", "janssen_partA_VL", "IARCHPV", "VAT08")) & !grepl("stage2", attr(config,"config"))) {
...
#} else {
  ...
  if (COR_postfix_list == "adhoc"){ # only for config: janssen_partA_VL and COR: D29VLvariant
  ...
  } else if (study_name == "VAT08" | attr(config,"config") %in% c("prevent19_stage2","azd1222_stage2")){
    ...
    for (COR_postfix in COR_postfix_list){
      ...
      # SET 1
      for (a in c("pseudoneutid50", if(attr(config,"config")!="prevent19_stage2") "bindSpike", if(attr(config,"config") %in% c("prevent19_stage2","azd1222_stage2")) "bindSpike_sub_nvx_stage2")){
        ...
        for (tp in tps_no_delta_over_tinterm_lb){
          ...
          tpLabel = tolower(tp)
          aLabel = gsub("_sub_nvx_stage2", " subset", a)
      ...
      for (assay_grp in assay_grp_list){
        ...
        if (((grepl("nab", COR_postfix) & sum(grepl("pseudoneutid50", assay_grp)))==0 | (grepl("bab", COR_postfix) & sum(grepl("bindSpike", assay_grp)))==0) & attr(config,"config")=="azd1222_stage2") next # for AZ stage 2
      ...
      for (bsero in c("non_naive")){
        if (attr(config,"config") %in% c("prevent19_stage2","azd1222_stage2")) next # prevent19_stage2 and azd1222_stage2 doesn't have non_naive
      ...
      for (bsero in c("naive")){
          ...
          for (trt in c("vac")){
              ...
              for (tp in if (attr(config,"config")=="prevent19_stage2") {c("Day35","Day35_sub")} else {tps_no_B_and_fold_change}){
                  ...
                  assay_n = ifelse(COR %in% c("D57azd1222_stage2_delta_nAb", "D57azd1222_stage2_severe_nAb"), 2,
                                   ifelse(COR %in% c("D57azd1222_stage2_delta_bAb", "D57azd1222_stage2_severe_bAb"), 8,
                                      length(assays)))
                  if (tp == "Day35_sub"){assay_n = 4; tp = "Day35"} # hard code to pool a subset pair plot for prevent19_stage2
                  tpLabel = ifelse(tp=="B", "Day1", tp)
