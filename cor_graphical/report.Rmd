...
if (study_name=="VAT08") {
  dat.longer.cor.subset.plot1 <- readRDS(here("data_clean", "longer_cor_data_plot1.rds"))
  Trialstage_val = unique(dat.longer.cor.subset.plot1$Trialstage)
} else {Trialstage_val = 999}
...
#} else {
  ...
  if (COR_postfix_list == "adhoc"){ # only for config: janssen_partA_VL and COR: D29VLvariant
    ...
    for (COR_postfix in COR_postfix_list){
      ...
      if (study_name == "VAT08") {
        if (COR_postfix=="D22D43omi" & Trialstage_val == 1) COR_postfix_label= paste0(" (Stage1)")
        if (COR_postfix=="D22D43omi" & Trialstage_val == 2) COR_postfix_label= paste0(" (Stage2)")
      }
