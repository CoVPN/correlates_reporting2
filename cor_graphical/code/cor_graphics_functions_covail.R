...
get_desc_by_group <- function(data, 
    ...
    data[which(data$time=="B"), "wt"]=data[which(data$time=="B"), "wt.D15"]
    data[which(data$time=="Day15"), "wt"]=data[which(data$time=="Day15"), "wt.D15"]
    data$wt[data$assay == "pseudoneutid50_MDW"] <- NA
    ...
    dat_stats <-
        ...
        group_by_at(group) %>%
        mutate(counts = n(),
               #num = ifelse(all(is.na(wt)), NA, sum(response * wt, na.rm=T)),
               num = sum(response * wt, na.rm=T),
               denom = sum(wt, na.rm=T),
               #denom = ifelse(all(is.na(wt)), NA, sum(wt, na.rm=T)),
...
#' @param ylim.1 y-axis limit for fold-rise
#' @param ylim.2 y-axis limit
#' @param ybreaks.1 y-axis breaks for fold-rise
#' @param ybreaks.2 y-axis breaks
...
#' @param strip.y.text.size font size for y-axis strip label
...
#' @param pool_label pooling lable add to the title
#' @param label_angle angle for x-axis label
#' @param label_hjust 
#' @param ncol legned column
#' @param nrow legned row 
f_case_non_case_by_time_assay <- 
    ...
    plot_theme <- theme_bw(base_size = 25) +
        theme(plot.title = element_text(hjust = 0.5),
              axis.text.x = element_text(size = axis.x.text.size,angle = label_angle,hjust = label_hjust),
    ...
    p1 <- dat %>%
        mutate(time_label=case_when(time=="B"~"Baseline",
                                    time=="Day15"~"Day 15",
                                    time=="Delta15overB"~"Fold-rise in"))%>%
        ...
        purrr::map(function(d){
            ggplot(data = d, aes(x = cohort_event, y = value)) +
                ...
                scale_color_manual(name = "", values = chtcols[1:4], guide = "none") + # guide = "none" in scale_..._...() to suppress legend
                # geoms below will use another color scale
                new_scale_color() +
                geom_point(alpha=0.2, aes(color = .data[[pointby]], shape = .data[[pointby]]), position=position_jitter(width = 0.25, height = 0),
                ...
                # Whisker: Q3 + 1.5 IQR
                geom_text(aes(label = ifelse(RespRate!="","Rate",""), x = 0.4, y = 5.8), hjust = 0, color = "black", size = panel.text.size, check_overlap = TRUE) +
                geom_text(aes(x = cohort_event, label = RespRate, y = 5.8), color = "black", size = panel.text.size, check_overlap = TRUE) +
                ...
                scale_x_discrete(labels = c("Non-Cases","D22-D91 Cases","D92-D181 Cases","D22-D181 Cases"), drop=FALSE) +
                scale_y_continuous(limits = if(d$time=="Delta15overB") ylim.1 else ylim.2, 
                                    breaks = if(d$time=="Delta15overB") ybreaks.1 else ybreaks.2, 
                                    labels = scales::math_format(10^.x)) +
                # scale_y_continuous(limits = ylim.2, 
                #                    breaks = ybreaks.2, 
                #                    labels = scales::math_format(10^.x)) +
                labs(x = "Cohort", 
                     y = ifelse(d$time=="Delta15overB",
                                paste0("fold-rise in nAb ", toupper(unique(d$panel))," over baseline"), 
                                paste0("nAb ", toupper(unique(d$panel)))),
                     title = ifelse(d$time=="Delta15overB",
                                    paste(unique(d$time_label), "nAb",toupper(unique(d$panel)), "distributions", "by non-case/case",pool_label),
                                    paste("nAb",toupper(unique(d$panel)), "distributions at", unique(d$time_label), "by non-case/case",pool_label)),
                     #title = paste("nAb",unique(d$panel), "distributions at", unique(d$time_label), "by non-case/case",pool_label),
                     color = "Category", shape = "Category") +
                plot_theme +
                guides(color = guide_legend(ncol=ncol, nrow = nrow), shape = guide_legend(ncol = 1))
        })
    return(p1)
    }
