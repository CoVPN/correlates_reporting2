...
tlf <-
  list(
    ...
    tab_gmtr = list(
      table_header = "Geometric mean titer ratios (GMTRs) of Responses for the D15 or D29 or D91 value compared to the D1 value 
...
# assays_col <- assays_col[!grepl("S1|S2", assays_col)] 
assays_col <- c(config$primary_markers, config$secondary_markers, 
                "Bcd8_FS_BA.4.5.S", "Day15cd8_FS_BA.4.5.S", "Bcd8_IFNg.IL2_Wuhan.N", "Day15cd8_IFNg.IL2_Wuhan.N")
...
tab_assay_list <- function(x, subs, sub.by.add=NULL, comp.i=c("Non-Cases", "Cases")){
    ...
    ds.i <- ds %>% dplyr::filter(!!as.name(paste0("ph1.D15", ".tcell"[grepl("tcell", COR)]))==1, !!as.name(Trti)!="")
    ...
    rgm_case <- get_gm(ds.i, mag.v, subs, sub.by, strata=config.cor$WtStratum, weights=config.cor$wt, subset=config.cor$ph2, digits=n_GM) %>%
      rename(!!subs:=Group) %>% 
    ...
    rgmt_case <- get_rgmt(ds.i, gmt.v, subs, comp_lev=comp.i, sub.by, strata=config.cor$WtStratum, weights=config.cor$wt, subset=config.cor$ph2) %>% 
...
tab_assay_nevercase <- lapply(1:Trtn, tab_assay_list, subs="CaseNevercase", comp.i=c("Never-Cases", "Cases"))
...
tab_assay_status_nevercase <- lapply(1:Trtn, tab_assay_list, subs="CaseNevercase", sub.by.add="Naive", comp.i=c("Never-Cases", "Cases"))
