...
tlf <-
  list(
    ...
    # ),
    tab_age = list(
        table_header = "Age at Baseline in the Per-Protocol cohort by treatment group and in overall study",
        # table_footer = randomsubcohort,
        deselect = "subgroup",
        pack_row = "subgroup"
        # col1="7cm"
    ...
    tab_enr = list(
      table_header = "Participants in Per-Protocol cohort by site and case-control status, in each treatment group and in overall study",
      table_footer = "Month 18 immune responses were measured in all  cases with Month 18 samples available and in controls selected in a 5:1 control:case sampling ratio. 
	  Controls were sampled with frequency matching to the case based on age 
      category at Month 18 ($\\\\leq$ 14 years, > 14 years), treatment group, and study site category (Mizoram/non-Mizoram)",
      deselect = "subgroup",
      pack_row = "subgroup"
      # col1="7cm"
    ),
    # tab_tested = list(
    #   table_header = "Proportion (\\%) of cases and controls with ELISA data at Month 18"
    #   deselect = "Arm",
    #   pack_row = "Arm",
    ...
    tab_days = list(
      table_header = "Duration from receipt of first HPV vaccine (enrollment date) to Month 18 visit in the per-protocol cohort"
      # deselect = "Arm",
      # pack_row = "Arm"
    ),
    ...
    # case_plcb = list(
    #   table_header = "Antibody levels in the per-protocol cohort (placebo recipients)",
    #   table_footer =c(
    ...
    #   covariate strata, calculated using inverse probability weighting."),
    #   
    #   col_name = c("Marker", "N", "Resp rate", "GMT/GMC", "N",
    #                "Resp rate", "GMT/GMC", "Resp Rate\nDifference", "GMTR/GMCR"),
    #   header_above1 = c(" "=1, "Cases*" = 3, "Non-Cases/Control" = 3,
    #                     "Comparison" = 2)
    #   header_above2 = c(" "=2,
...
ds_s <- dat %>%
  mutate(
    AgeC = ifelse(AgeGroup=="10-14", "Age $\\leq$ 14", "Age > 14" ),
    Arm = factor(TrtGroup, 
                 levels = c('Single-dose','Two doses default (Days 1 and 60)',
                            'Two-dose (Days 1 and =180)','Three-dose (Days 1, 60 and =180)')),
    EnrolltypeC = case_when(enrolltype=="Case" ~ "Cases",
                            enrolltype=="Control" ~ "Non-cases/Controls",
                            TRUE ~ enrolltype),
    CaseC = case_when(enrolltype=="Case" ~ "Cases",
                      TRUE ~ "Controls"),
...
pos.cutoffs <- sapply(assay_metadata$pos.cutoff, as.numeric)
...
for (i in times){
  for (j in assays){
    ...
    if (!is.na(cutoff)){
      ds[, paste0(post, "Resp")] <- as.numeric(ds_s[, post] > log10(cutoff))
    }
    }
}
...
char_lev <- c(labels.age,
              ...
              "Age $\\leq$ 14", "Age > 14", 
              "Cases", "Non-cases/Controls", "Controls", "Cohort",
              'Ahmedabad','Ambilikkai','Barshi','Delhi','Hyderabad','Mizoram','Pune','Sikkim',
...
tab_dm <- bind_rows(dm_cat, dm_num) %>%
  ...
  select_at(c("subgroup", "Characteristics",
              grep("Single-dose" ,names(.), value = T),
              grep("Two doses" ,names(.), value = T),
              grep("Two-dose" ,names(.), value = T),
              grep("Three-dose" ,names(.), value = T),
...
# if ((Numberdays <- paste0("NumberdaysD1toD", config.cor$tpeak)) %in% names(ds)) {
  tab_days <- ds %>% 
    filter(!is.na(CaseC)) %>%
    mutate(dur=as.numeric(ymd(month18date)-ymd(enrolldate))) %>% 
           # Visit = paste("Day", config.cor$tpeak)) %>% 
    bind_rows(., mutate(., CaseC = "Total")) %>%
    group_by(CaseC) %>% 
    summarise(N=n(), dmed=median(dur, na.rm=T), iqr=IQR(dur)) %>% 
    select(` `=CaseC, 
...
# Case counts by availability of markers at baseline, d29, d57
  tab_tested <- ds %>% 
    filter(!is.na(CaseC)) %>% 
    group_by(CaseC) %>% 
    summarise(N=n(), n=sum(month18tested=="Yes"), prop=sprintf("%s/%s=%.1f%%", n, N, n/N*100)) %>% 
    select(` `=CaseC, `Month 18` = prop)
...
# Cases vs Non-cases
  ...
  subs <- "EnrolltypeC"
  comp.i <- c("Cases", "Non-cases/Controls")
...
rrdiff_case <- rpcnt_case %>% 
  ...
  pivot_wider(id_cols = c(subgroup, Visit, Marker, Ind),
...
tab_case <- full_join(rpcnt_case, rgm_case,
                      by = c("Group", "N", "Marker", "Visit")) %>% 
  pivot_wider(id_cols = c(Marker, Visit),
              names_from = Group, 
              values_from = c(N, rslt, `GMT/GMC`)) %>% 
  full_join(rrdiff_case, by = c("Marker", "Visit")) %>% 
  full_join(rgmt_case, by = c("Marker", "Visit"))
...
tab_case_ord <- c("Anti  IgG Score", setdiff(assay_metadata$assay_label_short, "Anti  IgG Score"))


tab_case <- tab_case %>% 
  select(Visit, Marker, `N_Cases`, `rslt_Cases`, 
         `GMT/GMC_Cases`, `N_Non-cases/Controls`, `rslt_Non-cases/Controls`, `GMT/GMC_Non-cases/Controls`,  
         rrdiff, `Ratios of GMT/GMC`) %>% 
  mutate(Marker=factor(Marker, levels=tab_case_ord)) %>% 
  arrange(Visit, Marker) %>% 
  mutate(Marker=gsub("Anti ", "Anti", Marker))
