...
tlf <-
  list(
    ...
    case_vacc_neg = list(
      ...
      header_above1 = c(" "=2, "Severe Cases*" = 3, "Non-Cases/Control" = 3,
                        "Comparison" = 2),
      header_above2 = c(" "=2,
...
tab_strtm <- ds %>% 
  ...
  summarise("Day {tpeak} Severe Cases":=sum(Case=="Cases", na.rm=T), 
            `Non-Cases`=sum(Case=="Non-Cases", na.rm=T)) %>% 
  pivot_longer(cols=c(!!as.name(paste("Day", tpeak, "Severe Cases")), `Non-Cases`)) %>% 
