...
if(TRIAL %in% c("janssen_pooled_mock", "janssen_pooled_EUA", "janssen_na_mock", "janssen_na_EUA", "janssen_la_mock", "janssen_la_EUA", "janssen_sa_mock", 'janssen_sa_EUA', "janssen_trial_EUA")) {
  ...
  ENSEMBLE <- TRUE
  keys <-  c("D29IncludeNotMolecConfirmed", "D29SevereIncludeNotMolecConfirmed", "D29ModerateIncludeNotMolecConfirmed")
 
} else if(TRIAL %in% c("janssen_na_partA", "janssen_sa_partA")) {
  
  ENSEMBLE <- TRUE
  keys <-  c("D29IncludeNotMolecConfirmed", "D29ModerateIncludeNotMolecConfirmed")
 
} else if(TRIAL %in% c("hvtn705")) {
...
for(short_key in keys){
  ...
  title0 <- paste0(short_key, " Plots and Tables with estimates and ", ci, " confidence intervals"," for day ", day, " markers")
  if(monotone) {
