...
analysis_data_file <- "vat08_combined_data_processed_20250417.csv"
```
...
```{r, cache = T}
# read data 
df <- read.csv(analysis_data_file)

dir.create(
  "raw_res_only_standardized",
  showWarnings = FALSE,
  recursive = TRUE
)
```
...
run_analysis_helper <- function(data,
  ...
  if (xor(is.null(placebo_incidence), is.null(placebo_se))) {
    stop(
      "`placebo_incidence` and `placebo_se` must either both be supplied or both be NULL.",
      call. = FALSE
    )
  }

  if (!is.null(placebo_incidence)) {
    if (length(placebo_incidence) != 1L ||
        !is.finite(placebo_incidence) ||
        placebo_incidence <= 0) {
      stop("`placebo_incidence` must be one positive finite number.", call. = FALSE)
    }
...
- **Updated 8/31/26: fixed bug which affected standard error estimates for the VE curves**
- **Updated 4/23/25: with new weights, only included Stage 2 results, and harmonized y-axis**
...
for (stage in c(2)){
  ...
  for (marker in names(spike_marker_list)){
    ...
    df_temp <- df %>%
      filter(!!rlang::parse_expr(spike_filter))
    
    df_marginal <- df %>%
      filter(!!rlang::parse_expr(marginal_filter))
  ...
  for (marker in names(neut_marker_list)){
    ...
    df_temp <- df %>%
      filter(!!rlang::parse_expr(neut_filter))
...
data_path <- normalizePath(analysis_data_file, mustWork = TRUE)

cat(
  sprintf("- **Code commit:** `%s`\n", commit_hash),
  sprintf("- **Analysis-ready data:** `%s`\n", data_path)
)
```
