...
# source("/Volumes/ahudson/Ensemble-stage2-correlates/Methods-CausalCumulativeIncidence.R")
...
# ensemble.dat$CalendarDateDose2Mid <- ifelse(ensemble.dat$CalendarDateDose2 > as.Date("2021-12-31") &
#                                               ensemble.dat$CalendarDateDose2 <= as.Date("2022-04-01"), 1, 0)
# ensemble.dat$CalendarDateDose2Late <- ifelse(ensemble.dat$CalendarDateDose2 > as.Date("2022-04-01"), 1, 0)
...
out.sev <- CumulativeIncidenceSummary(time = time.sev[complete], event = event.sev[complete],
                                     ...
                                     t.start = 1, label = "Severe-critical",
                                     path.prefix = paste0(spath, "/reports/"),
                                     path.suffix = paste0("severe_", Sys.Date()), y.axis.bound.risk = .25, 
...
out.any.mod <- CumulativeIncidenceSummary(time = time.any.mod[complete], event = event.any.mod[complete],
                                          ...
                                          t.start = 1, label = "Moderate to Severe-critical",
                                          time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                          path.prefix = paste0(spath, "/reports/"),
                                          path.suffix = paste0("moderate-to-severe_", Sys.Date()), y.axis.bound.risk = .25,
                                          time.axis.offset = 13)
out.any <- CumulativeIncidenceSummary(time = time.any[complete], event = event.any[complete],
                                      W = W[complete,],
                                      A = A[complete], t0 = t0, fit = fit.any,
                                      t.start = 1, label = "Infection, Any Symptomatology",
                                      path.prefix = paste0(spath, "/reports/"),
                                      path.suffix = paste0("any-infection_", Sys.Date()), y.axis.bound.risk = .25,
                                      time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
                                      time.axis.offset = 13)
...
#                                       t.start = 1, label = "Severe-critical",
#                                       time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
#                                       path.suffix = "severe-adap-axis-1-22-2026",
#                                       time.axis.offset = 13)
...
#                                           t.start = 1, label = "Moderate to Severe-critical",
#                                           time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
#                                           path.suffix = "moderate-to-severe-adap-axis-1-22-2026",
#                                           time.axis.offset = 13)
...
#                                       t.start = 1, label = "Infection, Any Symptomatology",
#                                       time.axis.label = "Time (Days after Second Ad26.COV2.S Dose)",
#                                       path.suffix = "any-infection-adap-axis-1-22-2026",
#                                       time.axis.offset = 13)
...
# write.csv(tab, "/Volumes/ahudson/Ensemble-stage2-correlates/EfficacyImmunoCorrelates/Efficacy-1-22-2026.csv")
write.csv(tab, paste0(spath, "/reports/Efficacy_", Sys.Date(), ".csv"))
