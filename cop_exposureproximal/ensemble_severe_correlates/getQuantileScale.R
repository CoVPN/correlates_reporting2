...
#### recompute weight for D71

dat$ph2.D71=dat$ph2.D29 & !is.na(dat[,vv.D71[ind.marker]])

dat$ph2.D71=dat$ph2.D71 & (!is.na(dat$NumberdaysD1toD71) & (dat$NumberdaysD1toD71-dat$NumberdaysD1toD29<=dat[,vv.endt]))
...
dat<-add.wt(dat,ph1="ph1.D29",ph2="ph2.D71",Wstratum="Wstratum",wt="wt.D71",verbose=FALSE)
...
for (i in oo.Z1){
  ...
  #  W3.truc<-rep(WE3[i],ll)
    datlong<-rbind(datlong,cbind(rep(i,ll),rep(RE[i],ll),TE.truc,XE.truc,SE.truc,rep(ZE[i],ll),rep(SE[i],ll),rep(0,ll),1:ll,rep(delta[i],ll),rep(weight[i],ll),rep(data$wt.D71[i],ll),
...
colnames(datlong)<-c('id','R','time','X','S','Z','eventime','delta','visit','event','weight','wt.D71','Age','BMI','Sex','Region')
...
dd$W1<-1
fit<-WeMix::mix(X~time + Age+Sex + (1|id), data=dd,weights=c('W1','weightScale'),cWeights=TRUE)
#fit<-mix(X~time + Age+Sex + (1|id), data=dd,weights=c('W1','weight'),cWeights=TRUE)

#fit<-mix(X~time + Age+Sex + (1|id), data=dd,weights=c('W1','wt.D71'),cWeights=TRUE)

coef.fix<-fit$coef
slope<-coef.fix[2]

nl<-c(length(unique(datlong.2[datlong.2$event==0,]$id)),length(unique(datlong.2[datlong.2$event==1,]$id)))

##########

XE.v<-XE0.v+slope*TE.v

if (ind.region<3){
#q<-quantile(XE.v[delta.v==0 & Region.v==ind.region],c(0.9,0.95,0.975),na.rm=T)
q<-quantile(XE.v[delta.v==1 & Region.v==ind.region],c(0.9,0.95,0.975),na.rm=T)

} else {
 #q<-quantile(XE.v[delta.v==0],c(0.9,0.95,0.975),na.rm=T)
 q<-quantile(XE.v[delta.v==1],c(0.9,0.95,0.975),na.rm=T)
 
}
   
save(X0.min,nl,q,slope,file=paste("output/XEQ_Scale_event",ind.event,"_marker",ind.marker,"_region",ind.region,".Rdata",sep=''))
