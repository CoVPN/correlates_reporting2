...
cv_h_w = function(Theta, n, ident_n, n_te, arg_ext, arg_int, arg_ext_te,
                  Y, Y_te, W, W_te,
                  lm_int_list, lm_ext_list, bw0_int, bw0_ext,
                  bw_ext_scale_list, bw_int_scale_list){
  ...
  risk_array = array(0, dim = c(length(bw_int_scale_list), length(bw_ext_scale_list), length(lm_ext_list), length(lm_int_list)))
  alpha_array = array(0, dim = c(length(bw_int_scale_list), length(bw_ext_scale_list),length(lm_ext_list), length(lm_int_list), nrow(ident_n)))
  ...
  for(bw_int_ind in 1:length(bw_int_scale_list)){
    ...
    for (lm_int_index in 1:length(lm_int_list)){
      ...
      Gamm_int = Gamm(K = W %*% Kint, lm = lm_int, n = n, ident_n = ident_n)
      ...
      for (lm_ext_index in 1:length(lm_ext_list)){
        ...
        for(bw_ext_ind in 1:length(bw_ext_scale_list)){
          ...
          alpha =  bridge_fn_rkhs(Kext = Kext, Kint = Kint %*% W, ident_n = ident_n, A = Kint %*% W %*% Y, Gamm = W %*% Gamm_int, lm = lm_ext, n = n)
          
          ### Compute H_norm
          norm_ext = sqrt(as.numeric(t(alpha) %*% Kext  %*% alpha))
          
          xi0 = W %*%(Y- Kext %*% alpha)
          norm_int = sqrt(t(xi0) %*% t(Gamm_int) %*% Kint %*% Gamm_int %*% xi0)/2
...
cv_g_w = function(Theta, n, ident_n, n_te, arg_ext, arg_int, arg_int_q, arg_ext_te,
  ...
  for(bw_int_ind in 1:length(bw_int_scale_list)){
    ...
    for (lm_int_index in 1:length(lm_int_list)){
      ...
      Gamm_int = Gamm(K = Kint %*% W %*% qS, lm = lm_int, n = n, ident_n = ident_n)
      ...
      for (lm_ext_index in 1:length(lm_ext_list)){
        ...
        for(bw_ext_ind in 1:length(bw_ext_scale_list)){
          ...
          alpha =  bridge_fn_rkhs(Kext = Kext, Kint = Kint  %*% W %*%  qS, ident_n = ident_n, A = A, Gamm = qS %*% W %*%  Gamm_int, lm = lm_ext, n = n)
...
pmtp = function(data, trt = "X", outcome = "Y", covariates = c("L"),
  ...
  if (!is.null(ind_S)) {
    ...
    message("'ind_S' not provided — assuming a value of 1 for all observations.\n")
  }
  ...
  ### Computing indicators of exposure values falling in q(S).
  ...
  for(cv_rep in 1:K_folds){
    ...
    if(show.prog){
      cat(paste("(n_tr, n_te, n_eval) =", nrow(tr_data), nrow(te_data), nrow(eval_data)), "\n")
    }
    ...
    if(is.null(bw0_H)){
      if(no_weights){
        ...
        bw0_H = wt_euclidean_bw(X = as.matrix(tr_data[,c("A", names_L, names_W)]), w = tr_data$w)
      }
    }
    if(is.null(bw0_G)){
      if(no_weights){
        ...
        bw0_G = wt_euclidean_bw(X = as.matrix(tr_data[,c("A", names_L, names_Z)]), w = tr_data$w)
      }
    }
    ...
    if(no_weights){
      ...
      Theta_Gh = 0.25 * n_te * K_Gh_te %*% inv.mat(W_te %*% K_Gh_te  + theta * log(n_te) * ident_n_te)
      obj.cv.h = cv_h_w(Theta=Theta_Gh, n =n_tr, ident_n=ident_n_tr, n_te= n_te,
                        arg_ext = h_arg_tr,
                        arg_int = g_arg_tr,
                        arg_ext_te = h_arg_te, Y = tr_data$Y,
    ...
    ### Checkpoint
    if(show.prog){
      cat("Estimation of the outcome bridge function complete.\n")
    }
    ...
    for(q_index in 1:length(policy)){
      ...
      if(no_weights){
        ...
        obj.cv.g = cv_g(Theta=Theta_Hg, n = n_tr, ident_n= ident_n_tr, n_te= n_te,
                        arg_ext = g_arg_tr,
                        arg_int = h_arg_tr,
        ...
        Theta_Hg = 0.25 * n_te * inv.mat(K_Hg_te) %*% inv.mat(K_Hg_te %*% W_te %*% qS_te + theta * log(n_te) * ident_n_te)
        obj.cv.g = cv_g_w(Theta=Theta_Hg, n = n_tr, ident_n= ident_n_tr, n_te= n_te,
                          arg_ext = g_arg_tr,
                          arg_int = h_arg_tr,
                          arg_int_q = h_mtp_arg_tr,
                          arg_ext_te = g_arg_te, qS = qS,
      ...
      ### Checkpoint
      if(show.prog){
        cat(paste0("Estimation of the treatment bridge function for policy ", q_index, " complete.\n"))
      }
  ...
  if(compute.TMLE.est){
    res2 = sapply(1:length(policy), FUN = function(x){
      c(sum(wt_val^2*DR_IF[,x]^2)/n_total, sum(wt_val^2*TMLE_IF[,x]^2)/n_total)
    })
    colnames(res2) = q_names
    rownames(res2) = c("One-step Est.Var", "TMLE Est.Var")
  }else{
    res2 = sapply(1:length(policy), FUN = function(x){
      sum(wt_val^2*DR_IF[,x]^2)/n_total
    })
...
print.proximal.mtp <- function(x, sign.level = 0.05) {
  ...
  est_names = c("Proximal One-Step", "Proximal TMLE")
